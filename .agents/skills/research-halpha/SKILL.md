...
For a strategy direction, state its intended decision or portfolio role and require plausible after-cost value material to the user's stated objective. Personal-maintenance fit does not itself favor low exposure, low volatility or low return. Apply the fit contract in `HALPHA-ALP-003`, and leave unspecified return, drawdown and leverage preferences unresolved.
...
Follow the evidence-engineering escalation in [Research Method and Evidence](references/research-method-and-evidence.md): begin with the cheapest source, coverage, usable-sample and conservative economic-plausibility check that can falsify the question without exposing reserved outcomes. Add bespoke parsing, independent replay, full audit machinery or execution-grade simulation only when the candidate survives or that integrity check itself can change the decision.
...
Before viewing ranked batch results, fix the hypothesis or strategy family, asset universe, windows, transformations, parameters or models, comparison metric, applicable cost cases and intended trial count. Save the complete compact configuration-to-result table or its durable external identity, including failed and manually inspected variants; do not turn a large search into a sequence of unrecorded one-off scripts or retain only the winning statistic or column.

When related questions search toward one eventual decision policy, carry forward the cross-question candidate-family and architecture search history and every previously exposed evaluation interval, including materially explored state definitions, routing, weighting, switching and unknown-state rules. A new folder or local `trial_count=1` does not reset that selection history or make exposed evidence independent again.

Before downloading public raw data, resolve an existing shared identity through `research/halpha_research_data.py` when the source semantics, interval and expected bytes are already known. New manifests record the root-relative data identity, source URL, period, bytes and SHA-256 rather than a machine or temporary-worktree absolute path. A missing shared file may be reacquired only from its recorded source and must match the expected identity before use; changed bytes fail explicitly. Do not dynamically import a sibling study merely to obtain data or generic I/O. Keep historical checkpoints, manifests and outcome-bearing files unchanged.

Keep generated bulk data out of Git when appropriate. Full trades, per-bar panels, complete large diagnostics and replay outputs are external by default; Git retains the compact result for every tested configuration, failures, selection scope, immutable external identity and rerun command rather than only the winner. For a continuously refreshed study, keep fixed-cutoff evidence that supports the reported conclusion in the question folder's `evidence/` when it is suitable for Git; write reconstructable derived/replay outputs, mutable latest state and temporary caches outside Git. Retain an immutable refresh snapshot only when its input bytes or cutoff changed or when fixed evidence cites it. Let a page or API fall back from the external latest state to fixed evidence, and do not let routine refreshes overwrite completed evidence.
...
When the overall strategy claim depends on state identification, conditional routing, or dynamic weighting or exposure adjustment, treat the complete input-to-action policy as the `STRATEGY_CANDIDATE`. Apply the state-policy contract in `HALPHA-ALP-003` when the candidate actually uses market state; do not invent a state layer for another dynamic policy. Diagnose applicable state information and components separately, but only the forward-only complete policy can support cross-regime or portfolio performance. Hindsight labels, full-sample smoothed states and per-segment winner selection are descriptive or oracle evidence, not a tradable policy.

"""Correct the proven replay of an already-owned Halpha fill.
...
def upgrade() -> None:
    ...
    evidence = connection.execute(
        sa.text(
            ...
            SELECT stop.stop_state_version_id, stop.environment_id,
                   ...
                   stop.started_at, stop.loss_latch_digest,
                   stop.content_digest, stop.release_rules,
                   ...
                   fill.venue_fact_id AS synthetic_fill_fact_id,
            ...
            JOIN LATERAL (
                SELECT fact.venue_fact_id, fact.received_at, fact.source_time,
                       fact.payload
                FROM halpha.venue_fact AS fact
                WHERE fact.environment_id = stop.environment_id
                  AND fact.source_class = 'EXTERNAL_UNCLAIMED'
                  AND fact.kind = 'FILL'
                  AND fact.payload ->> 'client_order_id'
                      = accepted.source_object_id
                  AND fact.payload ->> 'venue_order_ref'
                      = accepted.payload ->> 'venue_order_ref'
                  AND fact.payload ->> 'reconciliation' = 'true'
                  AND fact.payload ->> 'trade_id'
                      ~ '^S-[0-9a-f]+-[0-9a-f]+$'
                  AND fact.payload ->> 'liquidity_side'
                      = 'NO_LIQUIDITY_SIDE'
            ...
            JOIN LATERAL (
                ...
                WHERE fact.environment_id = stop.environment_id
                  ...
                  AND fact.payload ->> 'venue_order_ref'
                      = accepted.payload ->> 'venue_order_ref'
                  AND fact.payload ->> 'last_quantity'
                      = fill.payload ->> 'last_quantity'
                  AND fact.payload ->> 'order_side'
                      = fill.payload ->> 'order_side'
                ORDER BY fact.received_at, fact.venue_fact_id
                LIMIT 1
            ...
            JOIN LATERAL (
                ...
                WHERE fact.environment_id = stop.environment_id
                  ...
                  AND fact.received_at > fill.received_at
                ORDER BY fact.received_at, fact.venue_fact_id
                LIMIT 1
            ) AS position ON TRUE
    ...
    synthetic_client_order_id = str(
        evidence["synthetic_client_order_id"]
    )
    open_ids = {
    ...
    correction_id = str(
        uuid5(
            NAMESPACE_URL,
            f"urn:halpha:owned-fill-replay-correction:{_SYNTHETIC_STOP_ID}",
        )
    )
    release_rules = {
    ...
    release_rules["system_reconciliation_correction"] = {
        ...
        "synthetic_fill_fact_id": str(
            evidence["synthetic_fill_fact_id"]
        ),
        ...
        "reason": "NAUTILUS_MISSING_FILL_REPLAY_OF_PERSISTED_HALPHA_FILL",
    }
    fields = {

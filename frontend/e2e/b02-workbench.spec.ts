...
test("B02 plan and limited-control surfaces preserve authority and failure boundaries", async ({ page }, testInfo) => {
  test.skip(!fixturePassword, "HALPHA_BROWSER_FIXTURE_PASSWORD is required for the explicit B02 fixture.");

  await page.goto("/login");
  await expect(page.getByRole("heading", { name: "登录 Halpha" })).toBeVisible();
  await assertAccessible(page, testInfo, "login");
  ...
  await expect(page.getByText("DEMO 主要验证系统流程与机制")).toBeVisible();
  ...
  await expect(dialog).toBeVisible();
  await dialog.getByLabel("Reauthenticate with local owner password").fill(fixturePassword!);

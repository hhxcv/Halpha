...
export default function StrategyIntroduction({
  strategy,
  embedded = false,
  showEvidenceLimit = true,
}: {
  strategy: StrategySummary;
  embedded?: boolean;
  showEvidenceLimit?: boolean;
}) {
  const evidenceLimit = typeof strategy.economic_scope.evidence_limit === "string"
    ? strategy.economic_scope.evidence_limit
    : "";
  const qualification = typeof strategy.economic_scope.profitability_evidence === "string"
    ? strategy.economic_scope.profitability_evidence
    : "";
  return (
    <Box
      component="section"
    ...
    >
      ...
      {showEvidenceLimit && evidenceLimit && (
        <Alert severity="warning" variant="outlined" sx={{ mt: 1.5 }}>
          <strong>{qualification === "NO_POSITIVE_EXPECTANCY_EVIDENCE" ? "收益证据未支持：" : "证据边界："}</strong>
          {evidenceLimit}
        </Alert>
      )}
    </Box>

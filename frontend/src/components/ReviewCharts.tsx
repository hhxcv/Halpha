...
function formatChartPrice(value: number): string {
  return marketPrice(String(value));
}
  ...
  useEffect(() => {
    ...
    const markers = fills.flatMap<SeriesMarker<UTCTimestamp>>((fill) => {
      ...
      return [{
        ...
        text: `${entry ? "入场" : kind === "TAKE_PROFIT" ? "止盈" : kind === "PROTECTION" ? "止损" : kind === "EXTERNAL_ACCOUNT_CLOSURE" ? "外部平仓" : "退出"} ${marketPrice(String(fill.price ?? ""))}`,
      }];
    });

...
**语言版本：** zh-CN  
**批准人：** Halpha 项目所有者  
**接受时间：** 2026-07-16T12:45:19+08:00  
**替代版本：** HALPHA-ENG-002@v0.4.0  
**上位文档或条款：** HALPHA-ARC-001 v1.8.0；HALPHA-ENG-001 v1.6.0  
**直接依赖：** 无 L3 直接依赖；本文件把上位工程约束投影为组件使用契约  
**能力消费者：** HALPHA-ALP-002 v0.3.0；HALPHA-TRADEPLAN-002 v0.8.0；HALPHA-DAT-002 v0.6.0；HALPHA-CAP-002 v1.0.0；HALPHA-EXE-002 v1.0.0；HALPHA-OUT-002 v0.6.0；HALPHA-UX-002 v0.9.0；HALPHA-SYS-002 v0.7.0（不是反向直接依赖）  
**适用纵向约束：** HALPHA-SYS-001 v1.5.0  
**本文档负责：** 长期采用的核心技术组件及使用契约；量化运行、Web、数据、认证、前端、通知、Windows 宿主、备份和测试能力的第三方复用边界；Halpha 必须保留的最小产品语义与数值边界；依赖、构建、迁移、回退和组件失败结果  
...
Halpha 采用一套可锁定的 Python 后端技术线、一套 React 前端技术线、一个 PostgreSQL 产品数据库和一个 NautilusTrader 量化交易栈。成熟组件已经承担的能力不在 Halpha 内重写；可妥协的技术或策略细节按组件公开行为调整；只有计划激活、互斥额度、最大允许损失、持久动作唯一性、用户控制和事实权威等不可妥协语义保留最小自研。
...
| HTTP API、OpenAPI 与 ASGI 运行 | FastAPI、Uvicorn、Starlette | App 暴露版本化查询与命令；Starlette 承担会话 cookie、可信 Host 和通用中间件 | 采用组件 |
...
- 通用 job、工作流、队列、retry 或 state-machine 框架；它们会与 `PlanActivation`、`PlanEvent`、`PendingAction`、Notification 和领域状态推进形成第二生命周期；
...
SYS 声明的 Executor 创建一个 `TradingNode`，装载 Binance data client、execution client、Controller 和 Halpha 协调入口。Controller 按 TRADEPLAN 已持久的 `PlanActivation` 恢复、添加、停止或移除 Strategy；一个激活对应一个稳定 Strategy 身份，组件内部 Strategy 状态不是激活权威。

`HalphaStrategyAdapter(Strategy)` 消费 NautilusTrader 提供的 Bar、timer、指标和订单事件，显式调用不继承 Strategy 的纯代码逻辑并把不可变 `StrategyProposal` 送入 Halpha sink。纯逻辑不能取得提交、修改、取消或退出订单的写 API。HalphaCoordinator 依次调用 TRADEPLAN、CAP 与 EXE 的公开应用边界，分别形成 `PlanEvent`/`ProposedAction`、资金与权限检查结果和 `PendingAction`；各所有者只写自己的记录，协调器只汇总同一事务。提交后再由 HalphaCoordinator 调用同一适配器的私有已持久动作门和 NautilusTrader OrderFactory。该门只对 activation_id、UUID 与条款摘要精确匹配的 PendingAction 开放已资格化的单订单提交、查询与取消能力。组件内部 RiskEngine 可作第二技术检查，不能批准 Halpha 额度或覆盖拒绝。
...
启动与连续核对使用 NautilusTrader 的公开能力。每个未闭合且非 `USER_TAKEOVER` 激活一个 HalphaStrategyAdapter、且不注册框架外部订单技术认领，是唯一设计拓扑；重启后即使框架把已知订单标为 EXTERNAL 技术身份，私有已持久动作门也只能按原 activation_id/UUID 查询或取消，未知 UUID 不自动归属或撤销。精确组件配置由 L4 固定。组件合成事件、内部 portfolio 或 cache 不得自行计入激活损失、关闭责任或释放额度。组件不能证明外部结果时，EXE 保持原 `PendingAction` 未决并按稳定身份查询，不重发写动作。

L4 必须以关闭框架持久 cache 的黑盒重启验证证明普通与算法订单能按原 UUID 重新装入、查询、取消和核对终态，且未知 UUID 不被误撤。固定组件版本不能满足时，本契约拒绝；任何替代拓扑只能另起 L3/L4 提案评审，不能成为当前自动 fallback。
...
FastAPI 只公开 UX 所需版本化查询、命令和 OpenAPI schema；openapi-typescript 由该 schema 生成浏览器类型，openapi-fetch 调用同一接口。手写 DTO 或前端枚举不能形成平行语义。HTTP 成功只表示接口结果，领域效果由 `Receipt` 和相应业务记录说明。
...
Python 直接依赖通过 pip-tools 产生锁定结果，前端使用其包管理器锁文件；生产运行不使用浮动版本、启动时下载或动态插件。可部署发布组至少能识别源修订、依赖锁摘要、迁移头、策略登记摘要、App/Executor/前端产物摘要和非秘密配置摘要。构建身份不表示获准交易。
...
4. FastAPI/OpenAPI 生成的前端类型、Pydantic 校验和领域拒绝一致；数据库事务、唯一约束、并发版本和 Alembic 失败恢复可重复；
...
7. App、Executor、数据库连接和量化节点分别终止并恢复时，只从权威记录与核对恢复，不重复动作；Windows 单实例与启停行为可验证；
8. 锁定构建可重建，`pg_dump` 产物能在隔离目标以 `pg_restore` 恢复，迁移和回退保持历史身份可读；
9. React 页面、RJSF 表单、TanStack Query 过期处理、命令回流、Playwright 关键路径和 axe 可访问性检查符合 UX 契约。
...
本方案不增加进程、产品数据库、真实写路径、权威记录族或运行模式；新增组件分别替代既有自研计划，且每项只有一个消费者边界。组件不能资格化时优先缩小支持范围、停止或由用户在官方工具接管，不建设第二实现。

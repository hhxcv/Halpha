...
**语言版本：** zh-CN  
**批准人：** Halpha 项目所有者  
**接受时间：** 2026-07-19T11:57:24+08:00  
**替代版本：** HALPHA-ENG-002@v0.9.0  
**上位文档或条款：** HALPHA-ARC-001 v1.9.0；HALPHA-ENG-001 v1.9.0  
...
**本文档负责：** 长期采用的核心技术组件及使用契约；交易所模拟盘与真实资金写环境的同源构建、统一执行客户端路径、真实只读 profile 的能力裁剪、profile 允许差异和环境等价清单；量化运行、Web、数据、认证、前端、通知、Windows 宿主、备份和测试能力的第三方复用边界；Halpha 必须保留的最小产品语义与数值边界；工程观察的证据声明、分段继承、规则重解释、冻结基线和失效边界；依赖、构建、迁移、回退和组件失败结果  
**本文档不负责：** 重定义策略、计划、事实、资金、动作、复盘或交互业务语义；规定当前精确版本、制品哈希、参数、主机、凭据值、部署命令、建设顺序、短时门时长、长期观察时长、资格化结论或运行可用性  
...
工程观察必须先把每项可消费结论写成独立证据声明。声明至少绑定：不可改写原始事实及摘要、采集器和测量语义版本、观察起止时间与时间窗、源代码/依赖锁/迁移头/制品/非秘密配置/启动接线/进程身份中实际参与该声明的输入、这些输入形成的依赖闭包，以及允许消费该声明的具体决定。完整 `commit_sha` 必须作为源码来源追溯身份，但不能替代声明级依赖闭包，也不能单独决定全部证据是否重置；任何未绑定输入保持未知，不能由路径相同、进程仍在或旧 `BuildManifest` 推定一致。

至少分别表达以下四类声明，不得用其中一类冒充另一类：

1. 宿主与平台连续性：Windows 醒钟、休眠间隔、系统重启和构建无关的宿主条件；
2. 进程与精确运行时连续性：进程身份、启动接线、运行源码、依赖、迁移、制品、配置和持续健康行为；
3. 当前构建即时资格：当前源码上的自动、集成、Demo、故障、回退测试与启动冒烟；
4. 发布与真实资金长期资格：绑定当前精确构建完整闭包的持续运行及真实外部只读观察。

差异评估必须逐项声明形成以下一种结果：`NO_RESET`、`REEVALUATE_RAW_EVIDENCE`、`RESET_AFFECTED_SEGMENT` 或 `RESET_FULL_AFFECTED_BASELINE`。变化明确位于声明依赖闭包之外，且由差异、构建输入与运行接线证明不影响谓词时采用 `NO_RESET`；因此普通文档、索引或证据元数据变化不能仅因产生新提交就重置运行证据。规则变化只有在原始事实完整覆盖新谓词，且采样范围、时间窗、时钟、身份、采集器和测量语义没有变化时，才可采用 `REEVALUATE_RAW_EVIDENCE`，并生成引用原始证据摘要、旧新规则摘要、评估器版本和判定时间的新派生结论。缺失事实、身份歧义、采集器或测量语义变化采用受影响分段或受影响基线重采，不能补造事实。

进程代码、依赖、迁移、制品、运行配置、启动接线、进程拓扑或相关观察器语义变化时，当前构建必须重新取得受影响的进程与精确运行时连续性；旧构建的时长证据不得与当前构建即时测试或启动冒烟组合成“当前构建已经历同等时长”的结论。宿主与 Windows 醒钟等经声明证明和构建无关的原始事实可以继续消费，但只能证明其原声明。

L4 的建设继续门可以是点时、面向后续建设的组合决定：使用既有已资格化的平台连续性声明，加上当前构建即时资格、当前启动冒烟、真实写门关闭和无未隔离核心缺陷。该组合必须明确只许可另行授权的后续建设，不证明当前构建取得进程时长、发布或真实资金资格。资格形成后，无关变化不追溯撤销；证据篡改、身份不明、依赖闭包内缺陷或规则变化使原谓词失效时，暂停依赖该结论的未完成切片，不回退无依赖且已经独立验证的建设。

发布、部署或真实资金长期观察必须先冻结不可变精确构建基线，绑定完整 `commit_sha`、Python 与前端依赖锁摘要、数据库迁移头、参与运行的后端与前端构建制品摘要、非秘密运行配置摘要、观察器源码和工具摘要，以及进程身份。观察期间该基线及其依赖闭包必须只读；后续建设使用独立 branch/worktree、构建、配置、数据库和进程实例。后续提交不重置隔离且身份未漂移的旧构建观察，但新构建也不得继承该精确构建结论；触及闭包时按影响建立新基线。

观察器、检查点、日志、摘要、重解释结果和报告都是可删除、可重建的资格制品，不得新增产品记录族、持久 worker、常驻服务、业务状态机或第二事实权威。长期观察失败只使对应冻结构建失去受其约束的发布、部署或真实资金使用资格，并暂停依赖缺陷的建设范围；修复后建立新基线，不自动判废不依赖该缺陷的后续建设。
...
| 证据原始事实、身份、依赖闭包、采集器或测量语义不完整 | 对应声明保持未知；按影响重采分段或受影响基线，不把旧构建时长、当前即时冒烟或无关声明相互替代 |
| 长期观察身份漂移或失败 | 对应冻结构建不得取得或继续使用受该证据约束的发布、部署或真实资金资格；按影响重建基线，不撤销无依赖的后续建设 |
...
10. 工程观察把宿主/平台连续性、进程/精确运行时连续性、当前构建即时资格和发布/真实资金长期资格分为独立声明；重解释结果绑定原始事实、旧新规则与评估器身份，触及声明闭包时只重采受影响范围，旧构建时长不转移给当前构建；长期观察精确绑定 commit、锁、迁移头、构建制品、非秘密配置、观察工具与进程身份，观察基线只读且与后续 worktree 和运行实例隔离。

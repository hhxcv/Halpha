...
**语言版本：** zh-CN  
**批准人：** Halpha 项目所有者  
**接受时间：** 2026-07-19T02:23:28+08:00  
**替代版本：** HALPHA-ENG-002@v0.8.0  
**上位文档或条款：** HALPHA-ARC-001 v1.9.0；HALPHA-ENG-001 v1.8.0  
...
**本文档负责：** 长期采用的核心技术组件及使用契约；交易所模拟盘与真实资金写环境的同源构建、统一执行客户端路径、真实只读 profile 的能力裁剪、profile 允许差异和环境等价清单；量化运行、Web、数据、认证、前端、通知、Windows 宿主、备份和测试能力的第三方复用边界；Halpha 必须保留的最小产品语义与数值边界；长期观察冻结基线、证据依赖闭包和继承边界；依赖、构建、迁移、回退和组件失败结果  
...
## 5.4 长期观察冻结基线与证据继承【ENG-AUTO-BLD-004-REQ】

每次长期观察必须先冻结一份不可变基线，至少绑定完整 `commit_sha`、Python 与前端依赖锁摘要、数据库迁移头、参与运行的后端与前端构建制品摘要、非秘密运行配置摘要，以及观察器实际读取的源码和工具摘要。证据必须声明这些输入形成的依赖闭包、观察进程身份、起止时间和观察用途；任何未绑定输入均保持未知，不能由路径相同、进程仍在或旧 `BuildManifest` 推定一致。

观察运行期间，冻结基线及其依赖闭包必须只读。观察器若使用仓库 checkout 或 worktree 作为输入，该输入范围不得承载后续修改；后续建设必须在独立 branch/worktree 中进行，并保持构建、依赖、配置、数据库和进程身份不指向观察实例。后续提交不重置已经隔离且身份未漂移的观察，但也不得继承该观察结论。

只有变化明确位于已声明依赖闭包之外，并由差异、构建输入和运行接线证明不会影响被观察行为时，新版本才可继续引用原证据的未受影响部分。源代码、锁、迁移、制品、配置或观察工具触及依赖闭包时，必须按影响重新资格化；仅有相同语义说明、相似文件或时间连续不能转移证据。

长期观察器、检查点、日志、摘要和报告都是可删除、可重建的资格制品，不得新增产品记录族、持久 worker、常驻服务、业务状态机或第二事实权威。长期观察失败只使对应冻结构建失去受其约束的发布、部署或真实资金使用资格，并暂停依赖缺陷的建设范围；修复后建立新基线，不自动判废不依赖该缺陷的后续建设。
...
| 长期观察身份漂移、失败或证据闭包不完整 | 对应冻结构建不得取得或继续使用受该证据约束的发布、部署或真实资金资格；按影响重建基线，不撤销无依赖的后续建设 |
...
10. 长期观察证据精确绑定 commit、锁、迁移头、构建制品、非秘密配置、观察工具与进程身份；观察基线只读，后续 worktree 与运行实例隔离，触及依赖闭包的变化不能继承旧观察结论。

...
L0–L3 的最小归类单元是“文档 + `-RAT` 语义锚点”，且不得与另外两份索引重叠；新 L3 没有 `-RAT` 锚点时不以普通设计章节替代。L4 只收录当前建设选择的直接依据，以 YAML 稳定键路径归类，并链接到计划文件而不绑定易漂移的行号；它不升级为稳定产品语义。当前每个文档编号只记录最新中文 ACCEPTED 正文，不重复记录旧版、英文、bundle、archive 或历史参考稿。当前 ACCEPTED 集包含 ALP、TRADEPLAN、DAT、CAP、EXE、OUT、UX、SYS、ENG 九个领域的十一份 L3；当前阶段采用哪些设计及采用范围由 L4 选择。
...
| 成熟外部工具优先承担其擅长的任务：交易场所负责官方账户操作，专业图表负责深度视觉分析，新闻和数据源提供原始材料，研究环境支持探索；通用 AI 可在 ALP 已确定边界内承担非权威的任务形成、研究实现、运行编排、比较、筛选、反证、复核和材料整理，但不取得事实、经济证据判断、正式策略晋升、交易计划或真实动作权威 | [HALPHA-FLOW-001](L1/HALPHA-FLOW-001-core-workflows-and-user-journeys.zh-CN.md) | [3.1 外部工具分工依据【FLOW-TOOL-001-RAT】](L1/HALPHA-FLOW-001-core-workflows-and-user-journeys.zh-CN.md#31-外部工具分工依据flow-tool-001-rat) |
| 研究体系选择成熟本地组件、正式 NautilusTrader 量化栈、外部 AI 可调用的窄 JSON/CLI 契约和文件证据交接，以减少 live/backtest 漂移、供应商集成和常驻平台责任；开发者 AI 按不会作恶且尽最大努力、但仍可能犯错的可信开发者处理，研究竞争力和错误防护来自问题、预登记、审查、schema、自动校验、完整搜索、未暴露 holdout、重演与现实检查，不采用生命周期成本明显高于风险价值的研究安全隔离 | [HALPHA-ALP-003](L3/HALPHA-ALP-003-research-workspace-and-evidence-handoff.zh-CN.md) | [成熟本地组件与文件交接的选择依据【ALP-RSCH-CMP-001-RAT】](L3/HALPHA-ALP-003-research-workspace-and-evidence-handoff.zh-CN.md#8-成熟本地组件与文件交接的选择依据alp-rsch-cmp-001-rat) |
...
| EXE 的六项稳定执行语义是防重复和恢复责任所需的最小可验证分离，不要求六个物理记录族；提交、场所结果、保护和核对证据可以嵌入单一 ExecutionAction 记录族，但不能覆盖历史或混淆未知与责任 | [HALPHA-EXE-001](L2/HALPHA-EXE-001-execution-protection-reconciliation-and-recovery.zh-CN.md) | [稳定执行语义的选择依据【EXE-OBJ-001-RAT】](L2/HALPHA-EXE-001-execution-protection-reconciliation-and-recovery.zh-CN.md#稳定执行语义的选择依据exe-obj-001-rat) |
...
| Halpha 的产品、研究和文档本就由开发者使用 AI 共同建设；仅对 research 构造敌对 AI 假设不能覆盖项目整体失误风险，当前以审查、schema、测试、重演、证据复核、回退和真实资金前重验更直接限制善意错误，隔离只有在可观察风险的降低价值高于全生命周期成本时才有净价值 | [HALPHA-ENG-001](L2/HALPHA-ENG-001-ai-development-and-engineering-quality.zh-CN.md) | [1.1 可信 AI 与相称防错的选择依据【ENG-AID-001-RAT】](L2/HALPHA-ENG-001-ai-development-and-engineering-quality.zh-CN.md#11-可信-ai-与相称防错的选择依据eng-aid-001-rat) |
| 自然时间主要提高它实际覆盖声明的证据，不会自动提高独立后续切片的实现正确性；按声明依赖闭包保留未受影响的原始事实、以有界短时门继续建设、以当前精确构建长期观察约束发布和真实资金使用，既避免无关变更重复计时，也不把旧构建时长冒充当前构建资格 | [HALPHA-ENG-001](L2/HALPHA-ENG-001-ai-development-and-engineering-quality.zh-CN.md) | [5.2 分离建设等待与真实使用门的选择依据【ENG-OBS-001-RAT】](L2/HALPHA-ENG-001-ai-development-and-engineering-quality.zh-CN.md#52-分离建设等待与真实使用门的选择依据eng-obs-001-rat) |
| 一个项目所有者、一个仓库、一个产品发布组和少量工作包优先使用 Git branch/worktree、路径差异门、DAG 与串行 merge/PR；单一研究 venv、直接 harness、无状态 CLI、宿主文件锁、审查与既有测试链已覆盖当前可信 AI 的主要失误风险，只有经真实错误场景、错误成本和生命周期成本证明净价值为正时才增加更强隔离，避免引入 sandbox、父/worker、专用 IPC、服务、队列或数据库的过度复杂度 | [HALPHA-ENG-003](L3/HALPHA-ENG-003-monorepo-workspaces-and-parallel-integration.zh-CN.md) | [Git 原生工作区与串行集成的选择依据【ENG-MONO-CMP-001-RAT】](L3/HALPHA-ENG-003-monorepo-workspaces-and-parallel-integration.zh-CN.md#8-git-原生工作区与串行集成的选择依据eng-mono-cmp-001-rat) |

## 当前建设决策依据

| 决策或依据 | 文档 | 章节 |
|---|---|---|
| D00 已对齐授权有效性/运行资格分层、P0 MANUAL_PLAN_RESUME、CAP 单动作/四类停止和同进程 `/operations`；为最大化 Demo 对实盘机制的预验证价值，TRADEPLAN 只拥有 ProposedAction，EXE 唯一拥有 DEMO/LIVE 共用实现的环境限定 ExecutionAction，Demo 以系统机制证据为主且策略结论必须披露市场差异 | [HALPHA-PLAN-001](L4/HALPHA-PLAN-001-current-construction-plan.yaml) | [formalization_record](L4/HALPHA-PLAN-001-current-construction-plan.yaml) |
| P0 产品路径继续只建设本机 Web 与同进程有限操作入口；唯一例外是本计划显式登记、对 P0 产品闭环保持零 effects 的隔离研究轨。R00 的基础设施资格化不消费 B04 最终数据边界，因此在自身包契约满足后可通过独立 worktree/branch 与 B04/B05 包级并行；只有 R01 完成、政策冻结和 confirmation 边界固定局部等待 B04 最终数据边界，R02/R03 继续后置。上位只保留未来交互形态接入同一业务流的兼容边界，当前 L4 不定义 P1–P3，也不预建其他形态的认证、网络、同步或客户端方案 | [HALPHA-PLAN-001](L4/HALPHA-PLAN-001-current-construction-plan.yaml) | [planning_horizon / research_track.preexisting_data_boundary / parallelization_rule](L4/HALPHA-PLAN-001-current-construction-plan.yaml) |
| 当前组件能力取舍：订单保护以显式数量 `reduce_only` 代替 `close_position`；历史回测使用 reference-price/fee 代理并将资金费标为 `NOT_MODELED`，不自研结算器、不切换 2.0 RC、也不设自动盈利硬门；恢复只采用 `external_order_claims=None` 单一路径，任一资格化失败即阻断，不预建第二恢复拓扑 | [HALPHA-PLAN-001](L4/HALPHA-PLAN-001-current-construction-plan.yaml) | [component_selection / p0_order_profile_selection / historical_and_runtime_evidence](L4/HALPHA-PLAN-001-current-construction-plan.yaml) |
| 以正式化前设计为比较基线，删除重复自研后，权威持久记录族由 26 降至不超过 16、持久工作器类别由 5 降至不超过 2，其他结构计数不增加 | [HALPHA-PLAN-001](L4/HALPHA-PLAN-001-current-construction-plan.yaml) | [complexity_budget.comparison_baseline / complexity_budget.before](L4/HALPHA-PLAN-001-current-construction-plan.yaml) |
| 当前建设继续门以既有 8 小时 Windows 醒钟/宿主平台连续性声明，组合当前构建即时测试、启动冒烟、真实写门关闭和无未隔离核心缺陷；它只服务另行授权的建设继续，不声称当前构建已连续运行 8 小时。72 小时 Windows 与 7–14 日 LIVE_READ_ONLY 仍绑定当前精确构建并服务发布与真实资金资格 | [HALPHA-PLAN-001](L4/HALPHA-PLAN-001-current-construction-plan.yaml) | [construction_packages.B04.construction_continuation_gate / long_observation](L4/HALPHA-PLAN-001-current-construction-plan.yaml) |

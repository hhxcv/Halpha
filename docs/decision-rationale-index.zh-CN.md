...
| 研究体系选择成熟本地组件、正式 NautilusTrader 量化栈、外部 AI 可调用的窄 JSON/CLI 契约和文件证据交接，以减少 live/backtest 漂移、供应商集成和常驻平台责任；研究竞争力来自问题、预登记、证伪、完整搜索、未暴露 holdout 与现实检查，完整数据能力在模型外沙箱不能实证时保持禁用 | [HALPHA-ALP-003](L3/HALPHA-ALP-003-research-workspace-and-evidence-handoff.zh-CN.md) | [成熟本地组件与文件交接的选择依据【ALP-RSCH-CMP-001-RAT】](L3/HALPHA-ALP-003-research-workspace-and-evidence-handoff.zh-CN.md#8-成熟本地组件与文件交接的选择依据alp-rsch-cmp-001-rat) |
...
| 一个项目所有者、一个仓库、一个产品发布组和少量工作包优先使用 Git branch/worktree、路径差异门、DAG 与串行 merge/PR；独立研究环境和无状态 CLI 解决依赖与 AI 吞吐问题，资格化外部沙箱、受限 stdio、宿主文件锁和既有测试链比 monorepo 编排平台、模型运行时、服务、队列或数据库具有更低净复杂度 | [HALPHA-ENG-003](L3/HALPHA-ENG-003-monorepo-workspaces-and-parallel-integration.zh-CN.md) | [Git 原生工作区与串行集成的选择依据【ENG-MONO-CMP-001-RAT】](L3/HALPHA-ENG-003-monorepo-workspaces-and-parallel-integration.zh-CN.md#8-git-原生工作区与串行集成的选择依据eng-mono-cmp-001-rat) |

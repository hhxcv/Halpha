...
| P0 产品路径继续只建设本机 Web 与同进程有限操作入口；唯一例外是本计划显式登记、对 P0 产品闭环保持零 effects 的隔离研究轨，R00–R02 在 B04 完成后可通过独立 worktree/branch 与 B05 包级并行，R03 后置；上位只保留未来交互形态接入同一业务流的兼容边界，当前 L4 不定义 P1–P3，也不预建其他形态的认证、网络、同步或客户端方案 | [HALPHA-PLAN-001](L4/HALPHA-PLAN-001-current-construction-plan.yaml) | [planning_horizon](L4/HALPHA-PLAN-001-current-construction-plan.yaml) |

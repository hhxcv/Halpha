...
**Parent Documents:** HALPHA-CON-001, HALPHA-DOC-001, HALPHA-VIS-001, and HALPHA-FLOW-001  
**This Document Governs:** enduring technical realization of VIS and FLOW; quality tradeoffs; overall logical form; dependency direction; authoritative state; simulated and real trading-action paths; environment isolation; restart continuity; and architectural concerns for the responsibility map  
...
4. Exchange-simulation and real-funds actions initiated by Halpha are both owned by EXE and use the same action responsibility, CAP checks, external submission, and reconciliation chain. The real-funds environment changes only the environment, account, and least-privilege credentials; it adds no second admission layer or external state file.
5. Halpha does not decide the user's total personal investment. It enforces only the configured funds-use caps and scope, applicable decision basis, pre-action checks, authorization, and stopping. It neither reserves funds for future actions nor builds multi-step funds approval or a separate production-admission product.
...
7. Research, AI, reporting, management, and configuration capabilities must not become required synchronous dependencies for real-action submission, stopping, reconciliation, or restart continuity.
8. The responsibility map states only who owns stable meaning; it does not require same-named modules, services, or equal implementation investment.
...
| 6 | Implementation of financial risk controls | Failure to enforce the applicable decision basis, funds-use caps and scope, current activation scope, amount, or exposure checks |
...
When a third-party component fully provides an internal module capability, Halpha design still determines the semantic owner and public boundary, but no corresponding Halpha code module, component state machine, or general wrapper layer is required. The specific component adopted for the long term and its usage contract enter the applicable L3; the exact version, current configuration, and validation results enter L4.
...
Horizontal business responsibilities express continuous business semantics; vertical responsibilities express cross-business constraints or support boundaries. They guide module boundaries but do not prescribe one code module, process, or service per responsibility. Runtime assurance comes jointly from mature technology choices, SYS, ENG, and the relevant business owners.
...
Action preparation → funds-use caps-and-scope and current-activation authorization check → execution → external-fact reconciliation → outcomes and interaction
~~~

Research, AI, reporting, management, and configuration functions must not become synchronous dependencies for real-action submission, stopping, reconciliation, or restart continuity.
...
# 5. Plan Activation and Execution Actions 【ARC-ACT-001】

## 5.1 Identifiable Decisions, Explicit Enablement, and Stopping

An enabled Trading Plan, the funds-use caps and machine-execution scope of this activation, and stopping, exit, and takeover decisions must all be identifiable, with historical decisions distinguished from the currently applicable decision. A material change creates a new version or decision under the applicable semantic owner's rules; content that supported a real action must not be rewritten silently.

The machine-execution scope must identify its object, boundary, duration, and failure outcome when the plan is activated. An interface, program, AI output, existing session, or ad hoc confirmation cannot create or expand that scope. Stopping and narrowing scope take effect immediately. Only the User changing and reactivating the plan may raise a funds-use cap, expand scope, or restore risk-adding capability.
...
or an explicit user instruction to cancel, protect, reduce, or exit that can be shown not to increase or transform risk
→ read the current environment, key facts, this activation's funds-use caps and machine-execution scope, and stopping, exit, and takeover decisions
→ confirm that the current action belongs to a valid activation or is an explicit contraction instruction for an existing responsibility that is shown not to add risk
→ Halpha completes checks of caps, scope, activation authorization, and stopping under CAP rules
→ before external writing, EXE durably records one environment-qualified action responsibility
...
Every permitted source in exchange simulation and real-funds environments MUST enter the same main chain. Plan activation is the only User-authorization entry for machine actions that add risk. Protection, cancellation, reduction, and exit instructions for existing responsibilities enter the same chain only when shown not to increase or transform risk. The chain contains no independent funds permission, reservation for future funds, per-action confirmation, final funds approval, or second real-funds admission layer. The CAP check determines whether the current action crosses the current environment's caps, scope, or activation authorization. Activation in simulation has no real-capital effect and cannot satisfy a real-funds check.

## 5.3 Local Transactions and Venue or Account Changes

A relational transaction makes only internal state atomic; it cannot form one transaction with an external venue or account change. Before external writing in any trading environment, Halpha MUST leave one recoverable action responsibility. When an external result is unclear, Halpha first queries by a stable external identity and does not resend blindly.
...
Historical research and historical market replay reuse only pure condition evaluation, plan semantics, and outcome analysis. Exchange simulation and real-funds paths MUST reuse the same TRADEPLAN→CAP→EXE→DAT→OUT business chain, external-action responsibility semantics, and venue-write implementation path, while environment, account, action, authorization, and result identities remain unambiguous.
...
- Direct tests proportionate to the impact MUST show that both environments use the same core business and venue-write implementation. L3 defines enduring permitted differences; L4 records current configuration and validation results.
...
- Moving from simulation to real funds requires completely new real-environment, account, action, and activation identities, and the user must select the real account, funds boundary, and plan-activation scope again. Simulation records MUST NOT be edited, promoted, copied, or migrated to perform that transition.
...
When a component fails, its capability is unknown, or earlier validation no longer applies, the affected path stops, becomes read-only, returns control to the User, or becomes explicitly unsupported according to the business impact. It must not switch automatically to a self-built implementation that is not also the sole design and sole validation target. Before restoring or replacing the component, Halpha must reconcile external facts, compatibility boundaries, and affected results again.

Startup and recovery must first read authoritative state, the current stopping decision, funds-use caps and scope, and the current activation's execution scope; they then connect read-only external sources, reconcile orders and positions, and handle unresolved real-action responsibilities. If a key fact, external result, or write authority is unclear, the affected scope remains stopped; only a contraction action explicitly directed by the user and shown not to increase or transform risk may continue. Restart does not replay a real action by default.

After the User stops new risk, revokes authority, exits, or takes over, the old run MUST NOT restore its own ability to add risk; existing risk continues to be protected or exited or is handed over to the User. A pure system restart MAY continue within the original scope only when the original authority, runtime continuity, identity, and external facts can all be shown to remain consistent. Otherwise it remains stopped and MUST NOT submit actions missed during downtime. CAP, EXE, DAT, and SYS own the concrete continuity, reconciliation, and new-run conditions.
...
Overall technology choices first directly adopt mature languages, frameworks, quantitative or domain components, relational databases, official venue adapters or mature adaptation capabilities, standard logs, off-the-shelf process management and backup, and a deployment form that one person can diagnose and recover, after direct validation for the intended use. When an off-the-shelf component differs from a replaceable technical choice, the design follows the component's actual capability and revalidates it rather than copying the old implementation detail in self-built code.

ENG establishes engineering trade-off requirements for capability search, difference classification, licenses, target platforms, upgrade and exit, and net complexity change. Under DOC, L3 records the specific component adopted for the long term, Halpha's usage contract, and the minimum supplementary boundary; L4 records the exact version, build identifier, current configuration, platform-validation results, and enablement state. ARC does not select a specific vendor or version.
...
Complexity first goes to trading judgment and planning, fact and real-action correctness, and clear controllable UX. SYS, CAP, ENG, governance, and supporting automation retain only the minimum scope needed by current real consumers. Stability alone does not justify a separate operations platform or expansion of a supporting domain.
...
# 12. Enduring Architectural Concerns for the Responsibilities 【ARC-L2D-001】

| Domain | Enduring architectural concern |
|---|---|
| ALP | Core judgment, economic evidence, and strategy quality; expand only for real research consumers |
| TRADEPLAN | Complete plan semantics that can execute and end |
| EXE | Uniqueness, external results, protection, unknowns, and reconciliation for environment-qualified external-action responsibility; simulation and real-funds environments use the same execution semantics |
...
| OUT | Start with short reviews and actionable issues; expand only from real samples |
| CAP | Funds-use caps and scope, activation authorization, action checks, stopping, and restart continuity |
...
The responsibility map describes semantic ownership, not a balanced investment plan. Every part required by an enabled core path must be correct; a part with no consumer is not built merely to complete a logical diagram. L4 records current support scope, products, instances, and implementation tradeoffs.

---

# 13. Handoff to Lower-Level Design 【ARC-HOF-001】

HALPHA-DOC-001 governs the general responsibilities and current-state recording boundaries of L2, L3, and L4. ARC hands off only its overall logical form, dependency direction, mature-capability-first principle, one authoritative relational database per transaction-record environment, one isolated external-write boundary per environment, shared simulation/real-funds execution implementation with isolated identities, and recovery order to applicable lower-level design. The relevant L2/L3 specifications refine domain, module, component-usage contract, interface, data, state, error, idempotency, concurrency, fencing, and test semantics for real consumers.

Under DOC, the enduring database product and third-party component usage contracts enter the applicable L3. Exact database versions and instances, third-party component versions and build identifiers, current configuration and validation results, processes and background workers, hosts and deployment locations, venues, and adapter support scope are recorded in L4. Design text, completed design, existing code, or one test cannot substitute for L4's current-capability and validation results.

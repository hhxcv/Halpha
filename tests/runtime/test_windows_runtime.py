...
def test_live_gate_maintenance_uses_the_executor_mutex_namespace(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured = {}
    sentinel = object()
    monkeypatch.setattr(windows_runtime, "require_process_identity", lambda sid: None)

    def acquire(**kwargs):
        captured.update(kwargs)
        return sentinel

    monkeypatch.setattr(windows_runtime, "_acquire_exclusive_mutex", acquire)

    result = acquire_live_gate_maintenance_mutex(
        name=r"Global\Halpha.Executor.BINANCE_LIVE.WriteOwner",
        executor_task_sid=BUILTIN_USERS_SID,
        maintenance_sid="S-1-5-21-1-2-3-1001",
    )

    assert result is sentinel
    assert captured["name"] == r"Global\Halpha.Executor.BINANCE_LIVE.WriteOwner"
    assert captured["owner_sid"] == "S-1-5-21-1-2-3-1001"
    assert captured["conflict_code"] == "LIVE_WRITE_EXECUTOR_MUST_BE_STOPPED"
    assert set(captured["grants"]) == {
        "S-1-5-18",
        "S-1-5-32-544",
        BUILTIN_USERS_SID,
        "S-1-5-21-1-2-3-1001",
...
def test_executor_maintenance_mutex_uses_maintenance_identity_and_shared_namespace(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        windows_runtime,
        "require_process_identity",
        lambda sid: identities.append(sid),
    )
    ...
    result = acquire_executor_maintenance_mutex(
    ...
    assert result is sentinel
    assert identities == ["S-1-5-21-1-2-3-1001"]
    assert captured == {
        "name": r"Global\Halpha.Executor.BINANCE_DEMO.WriteOwner",
        "owner_sid": "S-1-5-21-1-2-3-1001",
        "grants": {
            "S-1-5-18": windows_runtime.MUTEX_ALL_ACCESS,
            "S-1-5-32-544": windows_runtime.MUTEX_ALL_ACCESS,
            BUILTIN_USERS_SID: windows_runtime.MUTEX_ALL_ACCESS,
            "S-1-5-21-1-2-3-1001": windows_runtime.MUTEX_ALL_ACCESS,
        },
        "conflict_code": "EXECUTOR_MUST_BE_STOPPED_FOR_MAINTENANCE",
    }

...
def test_text_intel_processes_existing_raw_text_artifact(tmp_path: Path, capsys) -> None:
    ...
    assert "text_event_signals: analysis/text_event_signals.json" in stdout
    assert "event_intelligence_material: analysis/event_intelligence_material.md" in stdout
...
def test_text_intel_collects_configured_text_sources(tmp_path: Path, capsys, monkeypatch) -> None:
    ...
    assert _processor_statuses(manifest)["build_text_event_signals"] == "succeeded"
    assert _processor_statuses(manifest)["build_event_intelligence_material"] == "succeeded"

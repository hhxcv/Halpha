...
def _proposed(
    *,
    ...
    execution_context = (
        ...
        if kind is ProposedActionKind.EXIT
        else {}
    )
...
def test_all_selected_profiles_normalize_without_demo_specific_action_types() -> None:
    ...
    assert _action(cases[-1][0]).action_terms["exit_responsibility_role"] == (
        ExitResponsibilityRole.PRIMARY_EXIT.value
    )
    assert _action(cases[-1][0]).action_terms["position_side"] == "BOTH"
    assert "exit_responsibility_role" not in (
        _action(cases[-1][0]).action_terms["execution_context"]
    )
    assert "position_side" not in (
        _action(cases[-1][0]).action_terms["execution_context"]
    )


def test_hedge_side_exit_persists_the_exact_position_side() -> None:
    proposed = _proposed(
        kind=ProposedActionKind.EXIT,
        profile="REDUCE_OR_CLOSE_MARKET",
        order_type="MARKET",
        ...
        update={
            "execution_context": {
                "exit_responsibility_role": ExitResponsibilityRole.PRIMARY_EXIT.value,
                "position_side": "LONG",
            }
        }
    )

    action = _action(proposed)

    assert action.action_terms["position_side"] == "LONG"


def test_exit_action_requires_a_structured_responsibility_role() -> None:
    proposed = _proposed(
        kind=ProposedActionKind.EXIT,
        profile="REDUCE_OR_CLOSE_MARKET",
        order_type="MARKET",
        reduce_only=True,
    ).model_copy(update={"execution_context": {}})
...
@pytest.mark.parametrize(
    ...
    (
        ...
        _proposed(
            ...
            order_type="CANCEL",
            quantity=None,
            cancel_target={"client_order_id": "not-a-client-id", "endpoint": "ALGO"},
        ),
    ),
)
def test_profile_shape_rejects_ignored_or_ambiguous_terms(
...
def test_exit_source_replay_rejects_responsibility_role_conflict() -> None:
    event = _event(
        _proposed(
            kind=ProposedActionKind.EXIT,
            profile="REDUCE_OR_CLOSE_MARKET",
    ...
    action = _action(event.proposed_action)
    assert resolve_existing_action(action, plan_event=event) is action
    ...
    with pytest.raises(
        ExecutionActionConflict,
        match="DUPLICATE_IDENTITY_CONFLICT",
    ):
        resolve_existing_action(action, plan_event=conflict)
...
def test_submitting_crash_becomes_query_only_unknown_and_never_returns_ready() -> None:
    ...
    async_unknown = mark_submission_unknown(
        submitting,
        reason="VENUE_CALL_UNCERTAIN:VenueSubmissionUncertain",
        next_query_at=NOW + timedelta(seconds=10),
        observed_at=NOW + timedelta(seconds=2),
    )
    diagnosed_unknown = mark_submission_unknown(
        async_unknown,
        reason="VENUE_SUBMISSION_RESULT_UNKNOWN",
        next_query_at=NOW + timedelta(seconds=20),
        observed_at=NOW + timedelta(seconds=3),
    )
    assert diagnosed_unknown.unknown_reason == "VENUE_SUBMISSION_RESULT_UNKNOWN"
    assert diagnosed_unknown.state_version == async_unknown.state_version + 1

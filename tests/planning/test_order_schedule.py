...
from halpha.planning.order_schedule import (
    ...
    VenueOrderType,
    VenueTimeInForce,
    _legacy_v6_schedule_digest_payload,
...
def _legacy_v3_snapshot():
    ...
    return preview.model_copy(
        update={
            "compiler_version": "3",
            "schedule_digest": (
                "7de43cae93d6cadb8e3c7676720e9367"
                "1aa28c1b560cad5416d2769da987525b"
            ),
        }
    )


def _legacy_v4_snapshot():
    preview = _compile(
        OrderScheduleSpec(
            price_distribution=SinglePrice(limit_price="10"),
            amount_distribution=AmountDistribution(base_notional="20"),
            dynamic_rules=(CancelOnShockRule(invalidation_price="9"),),
        )
    )
    return preview.model_copy(
        update={
            "compiler_version": "4",
            "schedule_digest": (
                "f571e05d8da5579c1671a147859ec703"
                "d333d2bb245bf67cc9ad32fa0a88e58d"
            ),
        }
    )
...
def test_current_direct_catalog_rejects_incomplete_take_profit_allocation() -> None:
    spec = _protected_single_schedule(
        protection_policy=ProtectionPolicy(
            initial_stop=InitialStopSpec(distance_bps="100"),
            take_profit_ladder=TakeProfitLadderSpec(
                levels=(TakeProfitLevel(trigger_r="1", quantity_fraction="0.5"),)
            ),
        )
    )
    ...
    with pytest.raises(
        ValueError,
        match="DIRECT_EXECUTION_TAKE_PROFIT_FRACTION_TOTAL_INVALID",
    ):
        validate_direct_execution_schedule(spec)


def test_current_direct_catalog_rejects_more_than_four_take_profit_levels() -> None:
    spec = _protected_single_schedule(
        protection_policy=ProtectionPolicy(
            initial_stop=InitialStopSpec(distance_bps="100"),
            take_profit_ladder=TakeProfitLadderSpec(
...
def test_current_direct_catalog_accepts_four_complete_take_profit_targets() -> None:
    spec = _protected_single_schedule(
        protection_policy=ProtectionPolicy(
            initial_stop=InitialStopSpec(distance_bps="100"),
            take_profit_ladder=TakeProfitLadderSpec(
                levels=(
                    TakeProfitLevel(trigger_r="1", quantity_fraction="0.4"),
                    TakeProfitLevel(trigger_r="2", quantity_fraction="0.3"),
                    TakeProfitLevel(trigger_r="3", quantity_fraction="0.2"),
                    TakeProfitLevel(trigger_r="4", quantity_fraction="0.1"),
                )
            ),
        )
    )
...
def test_entry_reprice_requires_one_explicit_gtc_limit_order() -> None:
    rule = RepriceEntryRule()
    compatible = _protected_single_schedule(
        ...
        venue_policy=VenueOrderPolicy(
            ...
            time_in_force=VenueTimeInForce.GTC,
            post_only=True,
        ),
        dynamic_rules=(rule,),
    )

    validate_direct_execution_schedule(compatible)

    with pytest.raises(ValidationError, match="REPRICE_ENTRY_POLICY_CONFLICT"):
        _protected_single_schedule(
    ...
    with pytest.raises(ValidationError, match="REPRICE_ENTRY_POLICY_CONFLICT"):
        _protected_single_schedule(
            ...
            venue_policy=VenueOrderPolicy(
                ...
                expire_at=datetime(2026, 7, 24, tzinfo=UTC),
            ),
            dynamic_rules=(rule,),
        )
...
def test_new_direct_plan_requires_an_automatic_exit_path() -> None:
    spec = _protected_single_schedule(
        entry_program=EntryProgram(kind=EntryProgramKind.ONE_TIME),
    )
    ...
    with pytest.raises(
        ValueError,
        match="DIRECT_EXECUTION_AUTOMATIC_EXIT_REQUIRED",
    ):
...
def test_new_direct_plan_requires_an_explicit_entry_program() -> None:
    ...
    validate_direct_execution_schedule(legacy_compatible)
    with pytest.raises(
        ValueError,
        match="DIRECT_EXECUTION_ENTRY_PROGRAM_REQUIRED",
    ):
        validate_new_direct_execution_schedule(legacy_compatible)


def test_new_direct_plan_accepts_bounded_entry_reprice() -> None:
    spec = _protected_single_schedule(
        entry_program=EntryProgram(kind=EntryProgramKind.ONE_TIME),
        venue_policy=VenueOrderPolicy(
            order_type=VenueOrderType.LIMIT,
...
def test_time_sliced_market_entry_compiles_chronological_release_offsets() -> None:
    ...
    preview = _compile(spec, reference_price="100")

    assert preview.valid
    assert [leg.release_after_seconds for leg in preview.legs] == [30, 90, 150]
    assert [leg.price for leg in preview.legs] == [None, None, None]
    assert [leg.sizing_price for leg in preview.legs] == ["100", "100", "100"]
    assert [leg.requested_notional for leg in preview.legs] == ["10", "15", "20"]


def test_time_sliced_entry_requires_bounded_expiry_for_resting_orders() -> None:
    common = {
        "entry_program": EntryProgram(
            kind=EntryProgramKind.TIME_SLICED,
            slice_count=2,
    ...
    with pytest.raises(ValidationError, match="TIME_SLICED_RESTING_EXPIRY_REQUIRED"):
        OrderScheduleSpec(**common)
    with pytest.raises(ValidationError, match="TIME_SLICED_RESTING_EXPIRY_REQUIRED"):
        OrderScheduleSpec(**common, venue_policy=VenueOrderPolicy(post_only=True))

    resting = OrderScheduleSpec(
        **common,
        venue_policy=VenueOrderPolicy(post_only=True),
        dynamic_rules=(ExpireRemainingRule(after_seconds=30),),
    )
    assert resting.venue_policy.post_only is True
    assert resting.venue_policy.time_in_force is VenueTimeInForce.GTC
    validate_direct_execution_schedule(resting)
    with pytest.raises(
        ValueError,
        match="DIRECT_EXECUTION_TIME_SLICED_TIF_UNSUPPORTED",
    ):
        validate_new_direct_execution_schedule(resting)

    immediate = OrderScheduleSpec(
        **common,
        venue_policy=VenueOrderPolicy(time_in_force=VenueTimeInForce.IOC),
    )
    validate_new_direct_execution_schedule(immediate)

    with pytest.raises(ValidationError, match="TIME_SLICED_VENUE_POLICY_CONFLICT"):
        OrderScheduleSpec(
            **common,
            venue_policy=VenueOrderPolicy(
                time_in_force=VenueTimeInForce.GTD,
...
def test_event_entry_requires_a_market_event_condition() -> None:
    with pytest.raises(ValidationError, match="EVENT_TRIGGER_REQUIRED"):
        OrderScheduleSpec(
            entry_program=EntryProgram(kind=EntryProgramKind.EVENT_TRIGGERED),
            price_distribution=SinglePrice(limit_price="100"),
            amount_distribution=AmountDistribution(base_notional="20"),
            protection_policy=ProtectionPolicy(
                initial_stop=InitialStopSpec(distance_bps="100"),
...
def test_cutoff_does_not_change_executable_schedule_or_rules_digest() -> None:
    ...
    assert first.compiler_version == "7"
...
def test_venue_policy_conflicts_are_rejected() -> None:
    ...
    with pytest.raises(ValidationError, match="extra_forbidden"):
        VenueOrderPolicy(display_quantity=None)
...
def test_legacy_v2_snapshot_remains_verifiable_without_recompilation() -> None:
    validate_order_schedule_snapshot(_legacy_v2_snapshot())


def test_legacy_v3_snapshot_remains_verifiable_after_rule_model_upgrade() -> None:
    validate_order_schedule_snapshot(_legacy_v3_snapshot())


def test_legacy_v4_snapshot_remains_verifiable_after_rule_model_upgrade() -> None:
    validate_order_schedule_snapshot(_legacy_v4_snapshot())


def test_legacy_v6_snapshot_remains_verifiable_after_reprice_upgrade() -> None:
    preview = _compile(
        _protected_single_schedule(
            entry_program=EntryProgram(kind=EntryProgramKind.ONE_TIME),
            protection_policy=ProtectionPolicy(
    ...
    payload = _legacy_v6_schedule_digest_payload(
        schedule_spec=preview.schedule_spec,
        preprotected_parallel_supported=preview.preprotected_parallel_supported,
        venue_ref=preview.venue_ref,
        instrument_ref=preview.instrument_ref,
        direction=preview.direction,
        max_notional=preview.max_notional,
        reference_price=preview.reference_price,
        instrument_rules=preview.instrument_rules,
        requested_total_notional=preview.requested_total_notional,
        effective_total_notional=preview.effective_total_notional,
        legs=preview.legs,
    )
    legacy = preview.model_copy(
        update={
            "compiler_version": "6",
            "schedule_digest": content_digest(payload),
        }
    )

    validate_order_schedule_snapshot(legacy)


def test_legacy_v3_snapshot_rejects_a_post_v3_invalidation_rule() -> None:
    legacy = _legacy_v3_snapshot()
    changed_rule = legacy.schedule_spec.dynamic_rules[0].model_copy(
        update={"invalidation_price": "9"}
    )
    tampered = legacy.model_copy(
        update={
            "schedule_spec": legacy.schedule_spec.model_copy(
                update={"dynamic_rules": (changed_rule,)}
            )
        }
    )
...
def test_v7_snapshot_digest_covers_both_fixed_market_invalidation_bounds() -> None:
    preview = _compile(
        OrderScheduleSpec(
            price_distribution=SinglePrice(limit_price="10"),
            amount_distribution=AmountDistribution(base_notional="20"),
            dynamic_rules=(
                CancelOnShockRule(
                    ...
                    opportunity_missed_price="12",
                ),
            ),
        )
    )
    changed_rule = preview.schedule_spec.dynamic_rules[0].model_copy(
        update={"opportunity_missed_price": "13"}
    )
    tampered = preview.model_copy(
        update={
            "schedule_spec": preview.schedule_spec.model_copy(
                update={"dynamic_rules": (changed_rule,)}
            )
        }
    )

    assert preview.compiler_version == "7"
    with pytest.raises(ValueError, match="ORDER_SCHEDULE_SNAPSHOT_CORRUPT"):
        validate_order_schedule_snapshot(tampered)


def test_legacy_v4_snapshot_rejects_a_post_v4_opportunity_bound() -> None:
    legacy = _legacy_v4_snapshot()
    changed_rule = legacy.schedule_spec.dynamic_rules[0].model_copy(
        update={"opportunity_missed_price": "12"}
    )
    tampered = legacy.model_copy(
        update={
            "schedule_spec": legacy.schedule_spec.model_copy(
...
def test_legacy_v2_snapshot_rejects_cutoff_tampering() -> None:
...
def test_legacy_v2_snapshot_rejects_leg_tampering() -> None:
    legacy = _legacy_v2_snapshot()
...
@pytest.mark.parametrize("compiler_version", ("1", "8"))
def test_snapshot_rejects_an_unknown_compiler_version(
    ...
    with pytest.raises(ValueError, match="ORDER_SCHEDULE_COMPILER_UNSUPPORTED"):
        validate_order_schedule_snapshot(
            preview.model_copy(update={"compiler_version": compiler_version})
        )
...
def test_gtd_uses_explicit_evaluation_time_instead_of_stale_rules_cutoff() -> None:
    rules_cutoff = datetime(2026, 7, 23, tzinfo=UTC)
    evaluated_at = rules_cutoff + timedelta(minutes=30)
    spec = OrderScheduleSpec(
        price_distribution=PriceDistribution(
            lower_price="10",
            upper_price="20",
        ...
        venue_policy=VenueOrderPolicy(
            time_in_force=VenueTimeInForce.GTD,
            expire_at=evaluated_at + timedelta(minutes=5),
        ),
    )

    preview = compile_order_schedule(
        spec,
        _rules(source_cutoff=rules_cutoff.isoformat()),
        venue_ref="BINANCE_USDM",
        instrument_ref="BTCUSDT-PERP",
        direction=Direction.LONG,
        max_notional="100",
        schedule_ref="gtd-stale-rules-cutoff",
...
def test_order_schedule_evaluation_time_requires_timezone() -> None:
    with pytest.raises(
        ValueError,
        match="ORDER_SCHEDULE_EVALUATED_AT_TIMEZONE_REQUIRED",
    ):
        compile_order_schedule(
            OrderScheduleSpec(
                price_distribution=SinglePrice(limit_price="10"),
            ...
            direction=Direction.LONG,
            max_notional="100",
            schedule_ref="naive-evaluation-time",

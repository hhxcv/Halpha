...
from halpha.planning.adapter import (
    ...
    order_id_tag_for_activation,
    strategy_config_for_activation,
...
def test_order_id_tag_is_stable_and_unique_per_concurrent_activation() -> None:
    first = order_id_tag_for_activation("activation-a")
    second = order_id_tag_for_activation("activation-b")

    assert first == order_id_tag_for_activation("activation-a")
    assert first != second
    assert strategy_config_for_activation("activation-a").order_id_tag == first
    assert strategy_config_for_activation("activation-b").order_id_tag == second

...
def test_reprice_materializer_changes_only_bounded_price_and_attempt_identity() -> None:
    activation = _activation(
        OrderScheduleSpec(
    ...
    replacement = materialize_direct_schedule_reprice(
        ...
        base,
        attempt_index=1,
        replacement_price="100.2",
        reprice_index=1,
    )
    repeated = materialize_direct_schedule_reprice(
        activation,
        base,
        attempt_index=1,
        replacement_price="100.2",
    ...
    assert replacement == repeated
    assert replacement.execution_action_id != base.execution_action_id
    assert replacement.client_order_id != base.client_order_id
    assert replacement.proposed_action.price == "100.2"
    assert replacement.proposed_action.quantity == base.proposed_action.quantity
    context = replacement.proposed_action.execution_context["order_schedule"]
    assert context["attempt_index"] == 1
    assert context["retry_reason"] == "ENTRY_REPRICE"
    assert context["replacement_price"] == "100.2"
    assert context["reprice_index"] == 1


def test_post_only_retry_after_reprice_keeps_replacement_price() -> None:
    activation = _activation(
        OrderScheduleSpec(
            price_distribution=SinglePrice(limit_price="100"),
            amount_distribution=AmountDistribution(base_notional="20"),
    ...
    assert retry.proposed_action.price == "100.2"
    context = retry.proposed_action.execution_context["order_schedule"]
...
def test_reprice_materializer_rejects_off_tick_or_out_of_bounds(
    price: str,
    reprice_index: int,
) -> None:
    activation = _activation(
    ...
    with pytest.raises(ValueError, match="ORDER_SCHEDULE_REPRICE"):
        materialize_direct_schedule_reprice(
            activation,
            base,
            attempt_index=1,

...
def test_product_build_requires_built_frontend_bytes(tmp_path: Path) -> None:
    _product_tree(tmp_path)
    (tmp_path / "frontend" / "dist" / "index.html").unlink()
    settings = load_settings(ROOT / "config" / "halpha.example.toml")

    with pytest.raises(SourceIdentityError, match="SOURCE_IDENTITY_PATTERN_EMPTY"):
        calculate_product_build_id(tmp_path, settings)

# ruff: noqa: E402  # Repository bootstrap must precede Halpha imports.
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if venue_order is not None:
                latest_order = venue_order
                if expected_statuses is None or (
                    getattr(venue_order, "algoStatus", None)
                    in expected_statuses
                ):
                    return venue_order, attempt, transient_error_count
        if attempt < max_attempts:
            await asyncio.sleep(delay_seconds)
    return latest_order, max_attempts, transient_error_count
    ...
    try:
        ...
        if "ENTRY_STOP_MARKET" in strategy.orders:
            ...
            venue, attempts, transient_errors = await _query_algo_order_until_visible(
                ...
                max_attempts=20,
                delay_seconds=0.25,
                expected_statuses=frozenset({"CANCELED"}),
            )

"""Prepare the exact Binance BTCUSDT 1m historical research input."""
...
PREREGISTRATION_PATH = REPORT_ROOT / "historical-research-question.json"
EVIDENCE_PATH = ROOT / "build" / "qualification" / "historical-data.json"
...
class HistoricalDataError(RuntimeError):
    """Sanitized failure for historical input preparation."""
...
def _request_bytes(opener: Any, url: str) -> bytes:
    request = Request(url, headers={"User-Agent": "Halpha-Historical-Research/1"})
    try:
...
def main() -> int:
    ...
    try:
        ...
        evidence: dict[str, Any] = {
            "schema_version": 1,
            "operation": "HISTORICAL_DATA_PREPARATION",

"""Run the recorded-question development/holdout episode backtest."""
...
from tools.qualification.build_historical_catalog import (
    BAR_TYPE,
...
EVIDENCE_PATH = ROOT / "build" / "qualification" / "historical-backtest.json"
STARTING_EQUITY = Decimal("100000")
ONE_MINUTE = timedelta(minutes=1)


def _compute_backtest_activation_loss(
    *,
    realized_pnl: str,
    unrealized_pnl: str,
    funding: str,
    ...
    """Research-only loss projection; this is not a product runtime latch."""
    ...
    return max(
        -(realized + unrealized + funding_value - commission_value),
        Decimal("0"),
    )


def _update_backtest_loss_state(
    ...
    *,
    ...
    """Latch one reproducible backtest episode without promising product behavior."""
    ...
    evidence_unchanged = (
        ...
        and current.get("funding_query_cutoff")
        == funding_query_cutoff.isoformat()
    )
    updates = {
    ...
    if bool(current.get("max_loss_reached")):
        return current if evidence_unchanged else {**current, **updates}
...
SOURCE_FILES = (
    ...
    "tools/qualification/build_historical_catalog.py",
    "tools/qualification/run_historical_backtest.py",
)


class HistoricalBacktestError(RuntimeError):
    """Sanitized failure for the historical backtest boundary."""
...
class HistoricalEpisodeGateway:
    ...
    def __init__(
        ...
        *,
        ...
        self.activation = PlanActivation(
            activation_id=activation_id,
            environment_id=f"historical-{period_name}",
            environment_kind=EnvironmentKind.DEMO,
            authority_class=AuthorityClass.DEMO_VALIDATION,
            plan_version_ref="historical-plan-v1",
            account_ref="historical-research-account",
        ...
        self.boundary = ActivationCapitalBoundary(
            activation_id=activation_id,
        ...
        self.capital_state: dict[str, object] = {
            "activation_loss": "0",
    ...
    def _check_max_loss(self, observed_at: datetime) -> None:
        ...
        if realized is None or unrealized is None:
            return
        activation_loss = _compute_backtest_activation_loss(
            realized_pnl=str(_pnl_decimal(realized)),
            unrealized_pnl=str(_pnl_decimal(unrealized)),
            funding="0",
            commission=str(self.commission_total),
        )
        self.capital_state = _update_backtest_loss_state(
            self.capital_state,
            activation_id=self.activation_id,
            max_allowed_loss=self.boundary.max_allowed_loss,
            activation_loss=activation_loss,
        ...
        if bool(self.capital_state.get("max_loss_reached")):
            remaining = self.entry_filled_quantity - self.exit_filled_quantity
            if remaining > 0:
...
def _run_episode(
    *,
    ...
    activation_id = str(
        uuid5(
            NAMESPACE_URL,
            f"urn:halpha:historical:{period_name}:{decision_start.isoformat()}",
        )
    )
    valid_until = min(
    ...
    try:
        ...
        result = {
            ...
            "max_loss_reached": bool(gateway.capital_state.get("max_loss_reached")),
...
def main() -> int:
    ...
    partial_limit_text = os.environ.get("HALPHA_HISTORICAL_MAX_EPISODES")
    ...
    try:
        ...
        evidence: dict[str, Any] = {
            "schema_version": 1,
            "operation": "HISTORICAL_DEVELOPMENT_HOLDOUT",
            ...
            "performance_difference": {
                ...
                "holdout_interpretation": holdout_interpretation,
                "does_not_grant_trading_permission_or_profit_guarantee": True,
            },
            "technical_safety": {

...
DEFAULT_OUTPUT = ROOT / "build/qualification/b04-live-read-only-public-connectivity.json"
...
def _get_json(opener: Any, url: str) -> tuple[int, dict[str, Any]]:
    request = Request(
        url,
        headers={"User-Agent": "Halpha-B04-Public-ReadOnly-Probe/1"},
        method="GET",
    )
...
def probe(config_path: Path) -> dict[str, Any]:
    ...
    evidence: dict[str, Any] = {
        "schema_version": 1,
        "stage": "B04_BINANCE_PUBLIC_READ_ONLY_CONNECTIVITY",
        ...
        "source_sha256": {
            "tools/qualification/probe_b04_live_read_only_public_connectivity.py": (

...
DEFAULT_OUTPUT = ROOT / "build/qualification/product-demo-cycle.json"
CONFIG = ROOT / "config/halpha.toml"
LOG_DIRECTORY = ROOT / "build/qualification/runtime/product-demo-cycle"
...
PRODUCT_SOURCE_PATTERNS = (
    ...
    "tools/qualification/probe_product_demo_cycle.py",
    ...
    "tools/qualification/database_fixture.py",
)
    ...
    return (
        {
            ...
            "position_mode_enforced_as_product_precondition": False,
            "asset_mode_enforced_as_product_precondition": False,
            "margin_mode_or_leverage_enforced_as_product_precondition": False,
        },
        margin_mode,
        leverage,
    )
    ...
    *,
    ...
    for index, target_action_id in enumerate(reversed(tracked_target_actions)):
        ...
        try:
            await _cancel_target(
                ...
                reason_ref=f"PRODUCT_RECOVERY_CANCEL_{index}",
            )
        except Exception:
            continue
    position = await _position_amount(account_api)
    if position > 0 and anchor_action_id is not None:
        ...
        coordinated = runtime.coordinator.create_position_exit(
            ...
            reason_ref="PRODUCT_RECOVERY_EXIT",
            plan_event_id=str(uuid4()),
            execution_action_id=str(uuid4()),
            action_check=check,
            observed_at=observed_at,
...
def main() -> int:
    ...
    environment_id = f"product-demo-check-{uuid4()}"
    account_ref = f"demo-check-account-{uuid4()}"
    ...
    try:
        ...
        evidence["status"] = "BUILDING_PRODUCT_RUNTIME"
        runtime.build()

        async def product_flow() -> None:
            nonlocal terminal_proven
            flow_step = "REFERENCE_FACTS"
            account_api = None
            tracked_target_actions: list[str] = []
            ...
            try:
                ...
                checks["account_configuration_read_only_and_nonblocking"] = (
                    ...
                    and policy["position_mode_enforced_as_product_precondition"] is False
                    and policy["asset_mode_enforced_as_product_precondition"] is False
                    and policy["margin_mode_or_leverage_enforced_as_product_precondition"]
                    is False
                )
                ...
                with app_connection.transaction():
                    ...
                    command = build_command(
                        ...
                        scope={"reason": "PRODUCT_DEMO_COMPLETE_CYCLE"},
                        parameters={},
                ...
                proposal_basis = {
                    ...
                    "source_identity": (
                        f"{ids['activation_id']}:PRODUCT_ENTRY:{quote.ts_event}"
                    ),
                    ...
                    "reason_code": "PRODUCT_DEMO_CHECK",
                ...
                flow_step = "PROTECTION_AND_TAKE_PROFIT"
                ...
                flow_step = "ENTRY_RECONCILIATION"
                ...
                for index, target_id in enumerate(reversed(tracked_target_actions), start=1):
                    target, cancel = await _cancel_target(
                        ...
                        leverage=leverage,
                        current_position=quantity,
                        reason_ref=f"PRODUCT_PRE_EXIT_CANCEL_{index}",
                    )
                ...
                exit_result = runtime.coordinator.create_position_exit(
                    ...
                    position_quantity=canonical_decimal(position),
                    position_fact_ref=position_fact.venue_fact_id,
                    reason_ref="PRODUCT_CONTROLLED_EXIT",
                    plan_event_id=str(uuid4()),
                ...
                closure = runtime.coordinator.close_activation(
                    ...
                    fact_refs=fact_refs,
                    result_ref="PRODUCT_DEMO_REVIEW",
                    observed_at=cutoff,
                )
                ...
                evidence["flow_step"] = "COMPLETED"
                evidence["status"] = "COMPLETED"
            except Exception as exc:
                evidence["flow_step"] = flow_step
                errors.append(
                    f"PRODUCT_DEMO_FLOW_FAILED:{flow_step}:{_safe_error_reason(exc)}"
                )
        ...
        evidence["status"] = "RUNNING_PRODUCT_DEMO"

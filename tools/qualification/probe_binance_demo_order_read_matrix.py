...
from nautilus_trader.adapters.binance.futures.http.account import BinanceFuturesAlgoOrder
...
from nautilus_trader.adapters.binance.http.account import BinanceOrder
...
def _source_contract() -> dict[str, bool]:
    ...
    return {
        ...
        "ordinary_wrapper_exposes_bounded_time_window_and_limit": (
            "start_time: int | None" in ordinary_wrapper
            and "end_time: int | None" in ordinary_wrapper
            and "limit: int | None" in ordinary_wrapper
            and "startTime=start_time" in ordinary_wrapper
            and "endTime=end_time" in ordinary_wrapper
            and "limit=limit" in ordinary_wrapper
        ),
        "algorithm_wrapper_exposes_bounded_time_window_and_limit": (
            "start_time: int | None" in algorithm_wrapper
            and "end_time: int | None" in algorithm_wrapper
            and "limit: int | None" in algorithm_wrapper
            and "startTime=str(start_time)" in algorithm_wrapper
            and "endTime=str(end_time)" in algorithm_wrapper
...
async def _query_ordinary_history(
    account: BinanceFuturesAccountHttpAPI,
    *,
    symbol: str,
    start_ms: int,
    end_ms: int,
) -> tuple[list[BinanceOrder], list[int]]:
    orders: list[BinanceOrder] = []
    page_sizes: list[int] = []
    cursor_end_ms = end_ms
    for _ in range(MAX_HISTORY_PAGES):
        page = await account.query_all_orders(
            symbol=symbol,
            start_time=start_ms,
        ...
        if len(page) < PAGE_LIMIT:
            return orders, page_sizes

        create_times = [order.time for order in page]
        if not all(
            type(value) is int and value >= start_ms for value in create_times
        ):
            raise QualificationError(f"ORDER_HISTORY_CURSOR_FACT_INVALID:{symbol}")
        next_end_ms = min(create_times) - 1
        if next_end_ms < start_ms:
            raise QualificationError(f"ORDER_HISTORY_CURSOR_BOUNDARY_DENSE:{symbol}")
        if next_end_ms >= cursor_end_ms:
            raise QualificationError(f"ORDER_HISTORY_CURSOR_NO_PROGRESS:{symbol}")
        cursor_end_ms = next_end_ms
    raise QualificationError(f"ORDER_HISTORY_PAGE_BOUND_EXCEEDED:{symbol}")


async def _query_algo_history(
    account: BinanceFuturesAccountHttpAPI,
    *,
    symbol: str,
    start_ms: int,
    end_ms: int,
) -> tuple[list[BinanceFuturesAlgoOrder], list[int]]:
    """Read a bounded history window without relying on ignored page numbers."""

    orders: list[BinanceFuturesAlgoOrder] = []
    page_sizes: list[int] = []
    cursor_end_ms = end_ms
    for _ in range(MAX_HISTORY_PAGES):
        page = await account.query_all_algo_orders(
            symbol=symbol,
            start_time=start_ms,
            end_time=cursor_end_ms,
            limit=PAGE_LIMIT,
        ...
        raw_create_times = [order.createTime for order in page]
        if not all(
            type(value) is int and value >= start_ms for value in raw_create_times
        ):
            raise QualificationError(f"ALGO_HISTORY_CURSOR_FACT_INVALID:{symbol}")
        create_times = cast(list[int], raw_create_times)
        next_end_ms = min(create_times) - 1
        if next_end_ms < start_ms:
            raise QualificationError(f"ALGO_HISTORY_CURSOR_BOUNDARY_DENSE:{symbol}")
        if next_end_ms >= cursor_end_ms:
            raise QualificationError(f"ALGO_HISTORY_CURSOR_NO_PROGRESS:{symbol}")
        cursor_end_ms = next_end_ms
    raise QualificationError(f"ALGO_HISTORY_PAGE_BOUND_EXCEEDED:{symbol}")
...
async def _query(
    ...
    for symbol in SYMBOLS:
        ordinary, ordinary_page_sizes = await _query_ordinary_history(
            account,
            symbol=symbol,
            start_ms=start_ms,
            end_ms=cutoff_ms,
        ...
        algorithms, algo_page_sizes = await _query_algo_history(
            account,
            symbol=symbol,
            start_ms=start_ms,
            end_ms=cutoff_ms,
        ...
        instruments[symbol] = {
            "ordinary": {
                ...
                "pagination": "DESCENDING_END_TIME_CURSOR",
                "page_limit": PAGE_LIMIT,
                "page_sizes": ordinary_page_sizes,
                "record_count": len(ordinary),
                "short_page_terminates": ordinary_page_sizes[-1] < PAGE_LIMIT,
                "additional_page_requested": len(ordinary_page_sizes) > 1,
            ...
            "algorithm": {
                ...
                "pagination": "DESCENDING_END_TIME_CURSOR",
                "page_limit": PAGE_LIMIT,
                "page_sizes": algo_page_sizes,
                "short_page_terminates": algo_page_sizes[-1] < PAGE_LIMIT,
                "additional_page_requested": len(algo_page_sizes) > 1,

...
def _run_child_task(
    ...
    task_name = f"B04EmptyRestoreQualification-{os.getpid()}"
    definition = service.NewTask(0)
    definition.RegistrationInfo.Author = "Halpha Project Owner"
    definition.RegistrationInfo.Description = "Ephemeral B04 empty-restore qualification"
    ...
    action.Arguments = (
        "-m tools.qualification.verify_b04_restore_boundary "
...
def _parent(root: Path, config_path: Path, output: Path) -> dict[str, Any]:
    ...
    target_database = f"halpha_restore_b04_{os.getpid()}"
    ...
    try:
        ...
        checks = {
            ...
            "target_database_is_ephemeral": target_database.startswith("halpha_restore_b04_"),
        }
        report: dict[str, Any] = {
            "schema_version": 1,
            "observed_at": datetime.now(UTC).isoformat().replace("+00:00", "Z"),
            "stage": "B04_EMPTY_DATABASE_RESTORE",

"""Directly check Windows task, abnormal-restart, and PostgreSQL recovery behavior."""
...
DEFAULT_OUTPUT = Path("build/qualification/windows-fault-checks.json")
SOURCE_PATTERNS = (
    "config/halpha.toml",
    ...
    "tools/qualification/verify_windows_faults.py",
...
def qualify(
    ...
    *,
    ...
    report: dict[str, Any] = {
        "schema_version": 1,
        "operation": "WINDOWS_AND_DATABASE_FAULT_CHECKS",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if isinstance(exc, WindowsFaultQualificationError):
            ...
            reason = f"WINDOWS_FAULT_CHECK_FAILED type={type(exc).__name__}"
        report = {
            "schema_version": 1,
            "operation": "WINDOWS_AND_DATABASE_FAULT_CHECKS",
            "observed_at": datetime.now(UTC).isoformat().replace("+00:00", "Z"),

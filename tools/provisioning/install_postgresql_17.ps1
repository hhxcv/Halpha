...
if ([string]::IsNullOrWhiteSpace($InstallRoot) -or -not [IO.Path]::IsPathRooted($InstallRoot)) {
    throw 'POSTGRESQL_INSTALL_ROOT_ABSOLUTE_PATH_REQUIRED'
}
if ([string]::IsNullOrWhiteSpace($DataRoot) -or -not [IO.Path]::IsPathRooted($DataRoot)) {
    throw 'POSTGRESQL_DATA_ROOT_ABSOLUTE_PATH_REQUIRED'
}

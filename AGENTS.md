...
- Subagents inherit the parent task's authorization, semantic and path boundaries, effects, and exclusions. They do not authorize a new top-level outcome, worktree, commit, push, external write, or L4 advancement; the parent task remains the sole integrator and reviews the consolidated diff and validation. Concurrent writes are allowed only when semantic and file scopes do not overlap.
- Separately writable top-level tasks may run concurrently when their paths, product effects, credentials, databases, and external-write boundaries are independent; writable tasks use separate branches/worktrees and integrate serially. Research that writes only its own workspace and has no product-runtime, database, credential, or venue-write effect does not wait for a product milestone.
...
- Stay within the user-authorized outcome. L4 package boundaries are planning labels, not mandatory task boundaries: an authorized, no-external-write outcome may cover several small, directly testable slices when they share one result and can be verified together. Split the work when it introduces real external writes, must wait for external evidence, creates a path conflict, or would materially expand the authorized result. Never infer package completion or L4 advancement from code changes.

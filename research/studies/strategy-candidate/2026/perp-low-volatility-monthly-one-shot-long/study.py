...
STAGES = {
    "development": {
        "fetch_start": "2021-08-19T00:00:00Z",
...
CONFIG = {
    "strategy_id": "RESEARCH_PERP_VOL90_BOTTOM3_MONTHLY_ONE_SHOT_LONG_0P25X_V1",
    "direction": "LONG_ONLY",
    "formation_days": 90,
    "top_targets": 3,
    ...
    "diagnostics": {
        "vol60": {"kind": "low_volatility", "formation_days": 60, "top_targets": 3},
        "vol120": {"kind": "low_volatility", "formation_days": 120, "top_targets": 3},
        "bottom5": {"kind": "low_volatility", "formation_days": 90, "top_targets": 5},
        "highvol90": {"kind": "high_volatility", "formation_days": 90, "top_targets": 3},
        "mom90": {"kind": "momentum", "formation_days": 90, "top_targets": 3},
        "scheduled_long": {"kind": "scheduled_long", "formation_days": 90, "top_targets": 3},
    },
}
...
def command_checkpoint(_args: argparse.Namespace) -> None:
    ...
    if checkpoint_path.exists():
        ...
        if (
            ...
            or existing.get("universe", {}).get("categories") != CATEGORY_MEMBERS
            or existing.get("universe", {}).get("target_symbols") != TARGET_SYMBOLS
            or existing.get("files") != bound_file_hashes()
            or existing.get("environment") != environment_identity()
        ):
    ...
    payload = {
        ...
        "question": (
            "Does a low trailing 90-day realized volatility rank predict enough next-month absolute return "
            "among liquid current Binance perpetual targets to support a monthly LONG one-shot plan after "
            "retail costs, actual funding, capital hurdle, simple baselines, and sequential time evidence?"
        ),
        "evidence_boundary": (
            "The same constituent price bytes and a diversified spot low-volatility parent result are exposed. "
            "This exact current-perpetual, fixed-instrument, monthly one-shot conversion and its outcomes are "
            "unviewed. The fixed target list is a current-survivor rather than point-in-time market universe."
        ),
        "universe": {
...
def ensure_checkpoint() -> dict[str, Any]:
    ...
    if checkpoint.get("files") != bound_file_hashes():
        raise RuntimeError("checkpoint-bound method files changed")
    if checkpoint.get("environment") != environment_identity():
        raise RuntimeError("runtime environment differs from checkpoint")
    validate_universe_snapshot()
    return checkpoint
...
def command_fetch(args: argparse.Namespace) -> None:
    ...
    for number, symbol in enumerate(SYMBOLS, start=1):
        root = CACHE_ROOT / args.stage / symbol / "raw_lowvol90"
...
def circular_block_bootstrap(values: np.ndarray, block: int = 3, reps: int = 5000, seed: int = 20260722) -> list[float]:
...
def summarize(trades: pd.DataFrame) -> dict[str, Any]:
    ...
    eligible_targets = [value for value in target_stats.values() if value["trades"] >= 3]
    ...
    result["positive_category_count"] = int(sum(
        value["trades"] >= 3 and value["mean"] > 0 for value in result["by_category_base_after_hurdle"].values()
    ))
...
def command_analyze(args: argparse.Namespace) -> None:
    ...
    main_raw, main_excluded = build_trades(
        args.stage, bars, funding, "low_volatility", int(CONFIG["formation_days"]),
...
def gate_checks(stage: str, result: dict[str, Any]) -> dict[str, bool]:
    ...
    neighbors = ["vol60", "vol120", "bottom5"]
    checks = {
        ...
        "categories_at_least_three": main.get("categories", 0) >= 3,
        ...
        "positive_categories_at_least_three": main.get("positive_category_count", 0) >= 3,
        "beats_highvol90": base.get("cohort_mean_after_hurdle", -1.0) > result["diagnostics"]["highvol90"].get("scenarios", {}).get("base", {}).get("cohort_mean_after_hurdle", 1.0),
        "beats_mom90": base.get("cohort_mean_after_hurdle", -1.0) > result["diagnostics"]["mom90"].get("scenarios", {}).get("base", {}).get("cohort_mean_after_hurdle", 1.0),
...
def conclusion_for_failed_gate(result: dict[str, Any]) -> str:
    ...
    minimum_trades, minimum_targets = {
        "development": (30, 6), "evaluation": (15, 4), "confirmation": (10, 4),
    }[result["stage"]]
...
def command_conclude(_args: argparse.Namespace) -> None:
    ...
    handoff = {
        ...
        "decision": (
            ...
            "returns, rank ascending with symbol as deterministic tie-break, and propose LONG for ranks one to three."
        ),
        ...
        "framework_neutral_trace": [
            {"case": "rank_three", "target_volume_ok": True, "rankable_targets": 23, "vol90": 0.62, "feature_rank": 3, "proposal": "LONG_ENTRY_NEXT_MONTH_OPEN"},
            {"case": "rank_four", "target_volume_ok": True, "rankable_targets": 23, "vol90": 0.63, "feature_rank": 4, "proposal": "NO_ACTION"},
            {"case": "insufficient_universe", "target_volume_ok": True, "rankable_targets": 19, "vol90": 0.62, "feature_rank": None, "proposal": "NO_ACTION"},
        ],
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Low-volatility monthly one-shot LONG study")

...
CACHE_ROOT = Path(os.environ.get(
    "HALPHA_CATEGORY_MOMENTUM_CACHE",
...
CONFIG = {
    "strategy_id": "RESEARCH_CATEGORY_MOMENTUM_GATED_ONE_SHOT_LONG_7D_0P5X_V1",
    "direction": "LONG_ONLY",
    "formation_days": 7,
    "hold_days": 7,
    "top_categories": 2,
    "notional_fraction": 0.5,
    "peer_min_median_quote_volume_30d": 1_000_000.0,
    "target_min_median_quote_volume_30d": 10_000_000.0,
    "peer_max_count": 5,
    "own_category_min_peers_after_exclusion": 4,
    "other_category_min_peers": 5,
    "min_rankable_categories": 5,
    ...
    "diagnostics": {
        "formation_14d": {"kind": "category", "formation_days": 14, "hold_days": 7, "top_categories": 2},
        "hold_3d": {"kind": "category", "formation_days": 7, "hold_days": 3, "top_categories": 2},
        "top_1": {"kind": "category", "formation_days": 7, "hold_days": 7, "top_categories": 1},
        "own_momentum": {"kind": "own_momentum", "formation_days": 7, "hold_days": 7},
        "scheduled_long": {"kind": "scheduled_long", "formation_days": 7, "hold_days": 7},
    },
}
...
def command_checkpoint(_args: argparse.Namespace) -> None:
    ...
    payload = {
        ...
        "question": (
            "Does leave-one-out seven-day category momentum improve a user-fixed single-perpetual "
            "LONG one-shot plan after retail costs, actual funding, capital hurdle, simple baselines, "
            "and sequential time evidence?"
        ),
        "evidence_boundary": (
            "Many own-coin momentum and cross-sectional rules and most constituent price periods are exposed. "
            "This exact current-category, leave-one-out, fixed-instrument conversion and its outcomes are unviewed. "
            "The current-survivor category map is not point-in-time history."
        ),
        "universe": {
        ...
        "forbidden_after_checkpoint": [
            "selecting a favorable category, target, direction, window, holding period or rank cutoff",
            "lowering costs or capital hurdle", "opening a later stage after gate failure",
            "treating current classifications as historical point-in-time facts",
        ],
        "files": {
...
def category_scores(
    ...
    target_category = SYMBOL_TO_CATEGORY[target]
    for category, members in CATEGORY_MEMBERS.items():
        candidates: list[tuple[float, str, float]] = []
        for symbol in members:
            if category == target_category and symbol == target:
                continue
            med = median_volume.at[decision, symbol]
            ret = formation.at[decision, symbol]
            if pd.notna(med) and pd.notna(ret) and float(med) >= float(CONFIG["peer_min_median_quote_volume_30d"]):
                candidates.append((float(med), symbol, float(ret)))
        minimum = int(CONFIG["own_category_min_peers_after_exclusion"] if category == target_category else CONFIG["other_category_min_peers"])
        if len(candidates) < minimum:
            continue
        selected = sorted(candidates, key=lambda row: (-row[0], row[1]))[: int(CONFIG["peer_max_count"])]
        scores[category] = float(np.mean([row[2] for row in selected]))
    return scores
...
def signal_at(
    ...
    own_return = formation.at[decision, target]
    target_volume = median_volume.at[decision, target]
    common = {
        "own_formation_return": None if pd.isna(own_return) else float(own_return),
        "target_median_quote_volume_30d": None if pd.isna(target_volume) else float(target_volume),
        "category_score": None, "category_rank": None, "rankable_categories": 0,
    }
    if pd.isna(target_volume) or float(target_volume) < float(CONFIG["target_min_median_quote_volume_30d"]):
        return False, common
    if kind == "scheduled_long":
        return True, common
    if pd.isna(own_return):
        return False, common
    if kind == "own_momentum":
        return float(own_return) > 0.0, common
    scores = category_scores(target, decision, formation, median_volume)
    common["rankable_categories"] = len(scores)
    target_category = SYMBOL_TO_CATEGORY[target]
    if len(scores) < int(CONFIG["min_rankable_categories"]) or target_category not in scores:
        return False, common
    ranking = sorted(scores, key=lambda category: (-scores[category], category))
    rank = ranking.index(target_category) + 1
    common["category_score"] = scores[target_category]
    common["category_rank"] = rank
    return scores[target_category] > 0.0 and rank <= top_categories, common
...
def build_trades(
    ...
    formation = closes / closes.shift(formation_days) - 1.0
    ...
    decisions = pd.date_range(start, last_decision, freq="1D", inclusive="both")
    ...
    for symbol in TARGET_SYMBOLS:
        ...
        for decision in decisions:
            ...
            if pd.isna(entry_price) or pd.isna(exit_price) or float(entry_price) <= 0 or float(exit_price) <= 0:
                continue
            active, diagnostics = signal_at(kind, symbol, decision, formation, median_volume, top_categories)
            if not active:
                continue
            ...
            rows.append({
                "trade_id": f"{stage}-{kind}-{symbol}-{entry.strftime('%Y%m%d')}-{formation_days}-{hold_days}-{top_categories}",
                ...
                "formation_days": formation_days, "hold_days": hold_days, "top_categories": top_categories,
                **diagnostics,
...
def circular_block_bootstrap(values: np.ndarray, block: int = 28, reps: int = 5000, seed: int = 20260722) -> list[float]:
...
def command_analyze(args: argparse.Namespace) -> None:
    ...
    main_raw, main_excluded = build_trades(
        args.stage, bars, funding, "category", int(CONFIG["formation_days"]),
        int(CONFIG["hold_days"]), int(CONFIG["top_categories"]),
    )
    ...
    for name, spec in CONFIG["diagnostics"].items():
        diagnostic_raw, diagnostic_excluded = build_trades(
            ...
            int(spec["hold_days"]), int(spec.get("top_categories", CONFIG["top_categories"])),
        )
    ...
    payload = {
        ...
        "search_disclosure": {"selectable_primary_configurations": 1, "parameter_neighborhoods_not_selectable": 3, "simple_baselines_not_selectable": 2},
...
def gate_checks(stage: str, result: dict[str, Any]) -> dict[str, bool]:
    ...
    minimum_trades = 300 if stage == "development" else 150
    minimum_targets = 20 if stage == "development" else 15
    expected_years = {
        "development": ["2022", "2023"], "evaluation": ["2024"], "confirmation": ["2025", "2026"],
    }[stage]
    neighbors = ["formation_14d", "hold_3d", "top_1"]
    checks = {
        ...
        "beats_own_momentum": base.get("cohort_mean_after_hurdle", -1.0) > result["diagnostics"]["own_momentum"].get("scenarios", {}).get("base", {}).get("cohort_mean_after_hurdle", 1.0),
...
def conclusion_for_failed_gate(result: dict[str, Any]) -> str:
    main = result["main"]
    scenarios = main.get("scenarios", {})
    sample_ok = main.get("trades", 0) >= 150 and main.get("targets", 0) >= 15
...
def command_conclude(_args: argparse.Namespace) -> None:
    ...
    handoff = {
        ...
        "inputs": ["daily OHLCV including quote volume for target and fixed category members", "UTC bar close/open timestamps", "fixed category map"],
        "warmup_days": 30,
        "decision": (
            "At completed UTC day t, if target 30d median quote volume >=10m, rank at least five category scores. "
            "Each score is equal-weight 7d close return of up to five highest-30d-volume eligible peers; exclude target "
            "from its category. Propose LONG entry only when target category score >0 and ranks top two."
        ),
        "entry": "next UTC daily open after the decision", "exit": "UTC daily open exactly seven days after entry",
        "unknown_no_action": "missing, stale, discontinuous, invalid, insufficient peers/categories, or category mismatch",
        ...
        "framework_neutral_trace": [
            {"case": "top_two_positive", "target_volume_ok": True, "rankable_categories": 7, "target_category_score": 0.04, "target_category_rank": 2, "proposal": "LONG_ENTRY_NEXT_DAILY_OPEN"},
            {"case": "positive_but_rank_three", "target_volume_ok": True, "rankable_categories": 7, "target_category_score": 0.04, "target_category_rank": 3, "proposal": "NO_ACTION"},
            {"case": "missing_peer", "target_volume_ok": True, "rankable_categories": 4, "target_category_score": None, "target_category_rank": None, "proposal": "NO_ACTION"},
        ],
        "research_result_digest": {stage: stages[stage]["content_digest"] for stage in STAGES},
        "limitations": "Current-survivor and current-category map; not point-in-time full market; research result does not authorize product or trading use.",
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Category-momentum-gated one-shot LONG study")

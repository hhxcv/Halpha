...
SHORT_PARENT_PATH = HERE.parent / "perp-premium-daily-one-shot-short" / "study.py"
SPEC = importlib.util.spec_from_file_location("halpha_positive_premium_short_parent", SHORT_PARENT_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(f"cannot load positive-premium parent: {SHORT_PARENT_PATH}")
short_parent = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(short_parent)
daily_parent = short_parent.parent
engine = short_parent.engine

CACHE_ROOT = short_parent.CACHE_ROOT
TARGETS = list(short_parent.TARGETS)
STAGES = {
    "evaluation": {
    ...
    "confirmation": {
        "fetch_start": "2025-11-17T00:00:00Z",
        "start": "2026-01-01T00:00:00Z",
        "end_exclusive": "2026-07-01T00:00:00Z",
    },
}
CONFIG = {
    "strategy_id": "RESEARCH_PERP_PREMIUM1_TOP3_DAILY_ONE_SHOT_LONG_0P25X_OOS_V1",
    "direction": "LONG_ONLY",
    "top_targets": 3,
...
for module in [short_parent, daily_parent, engine]:
    module.HERE = HERE
...
def bound_file_hashes() -> dict[str, str]:
    return {
        ...
        "short_parent_study_py_sha256": engine.sha256_file(SHORT_PARENT_PATH),
...
def stage_authorized(stage: str) -> None:
    if stage == "confirmation":
        prior = HERE / "evaluation_gate.json"
        if not prior.exists() or engine.read_json(prior)["status"] != "PASS":
            raise RuntimeError("confirmation is sealed because evaluation gate is not PASS")
...
def command_checkpoint(_args: argparse.Namespace) -> None:
    ...
    payload = {
        ...
        "question": (
            "After an explicitly exposed 2024 discovery, does a positive prior-day Binance premium-index rank "
            "predict enough next-day positive absolute return among liquid current USD-M perpetual targets to "
            "support a 0.25x LONG one-shot plan in untouched 2025 and 2026H1 evidence after actual funding, retail "
            "costs, capital hurdle, baselines, robustness, and sequential gates?"
        ),
        "evidence_boundary": (
            "The 2024 opposite SHORT result exposed this LONG direction and is discovery-only. Related 2022-2023 "
            "premium research is also excluded. Only previously unopened 2025 evaluation and 2026H1 confirmation "
            "can qualify the candidate. The universe is current-survivor, not point-in-time."
        ),
        "universe": {
        ...
        "stages": STAGES,
        "config": CONFIG,
        "stage_open_rule": "evaluation -> confirmation; confirmation download requires evaluation PASS",
        "selection_disclosure": {"discovery_period": "2024", "selectable_primary_configurations": 1, "qualification_periods": ["2025", "2026H1"]},
        ...
        "forbidden_after_checkpoint": [
            "using 2022-2024 as qualification evidence",
            "selecting direction, premium/funding threshold, lookback, rank, target, day, cost, baseline, or gate from 2025-2026 outcomes",
            "opening confirmation after evaluation failure",
...
def command_fetch(args: argparse.Namespace) -> None:
    ...
    for number, symbol in enumerate(TARGETS, 1):
        root = CACHE_ROOT / "perp-premium-daily-long-oos" / args.stage / symbol
...
def signal_at(
    ...
    if kind in {"premium_premium", "funding_premium"} and float(value) <= 0:
        return False, common
...
def build_trades(
    ...
    opens, closes, quote_volume, premium_daily, funding_daily = daily_parent.make_inputs(stage, bars, funding, premium)
    if kind == "premium_premium":
        feature = premium_daily.rolling(lookback_days, min_periods=lookback_days).mean()
    elif kind == "funding_premium":
        feature = funding_daily.rolling(lookback_days, min_periods=lookback_days).mean()
    elif kind == "winner_momentum":
        ...
        for decision in decisions:
            ...
            rows.append({
                ...
                **diagnostics,
                ...
                "actual_funding_return": actual,
                "stress_funding_return": stress,
...
def command_analyze(args: argparse.Namespace) -> None:
    ...
    payload = {
        ...
        "search_disclosure": {
            "discovery_direction_configurations": 1,
            "selectable_primary_configurations": 1,
            "parameter_neighborhoods_not_selectable": 3,
            "simple_baselines_not_selectable": 3,
        },
        "trade_csv_sha256": engine.sha256_file(main_path),
        "diagnostic_csv_sha256": hashes,
...
def gate_checks(stage: str, result: dict[str, Any]) -> dict[str, bool]:
    ...
    minimum = {"evaluation": (75, 10, 60, 4), "confirmation": (50, 8, 40, 3)}[stage]
    years = {"evaluation": ["2025"], "confirmation": ["2026"]}[stage]
    neighbors = ["premium3", "premium5", "top5"]
    return {
...
def conclusion_for_failed_gate(result: dict[str, Any]) -> str:
    minimum = {"evaluation": (75, 10), "confirmation": (50, 8)}[result["stage"]]
    main = result["main"]
...
def command_conclude(_args: argparse.Namespace) -> None:
    ...
    if not all(value["status"] == "PASS" for value in gates.values()):
        raise RuntimeError("all untouched stages must PASS")
    ...
    handoff = {
        ...
        "warmup_days": 45,
        "decision": "Before UTC daily open require >=20 liquid complete targets; average three prior complete 8h premium closes, rank descending, and propose LONG only for positive premium ranks 1-3.",
        "entry": "next UTC daily open",
        "exit": "following UTC daily open; one full UTC day before reactivation",
        ...
        "framework_neutral_trace": [
            {"case": "positive_rank_three", "premium1": 0.0004, "feature_rank": 3, "rankable_targets": 23, "proposal": "LONG_ENTRY_NEXT_DAILY_OPEN"},
        ...
        "limitations": "2024 adaptive discovery excluded; current-survivor list; directional LONG is not arbitrage; no intraday execution/margin path; no product/trading authorization.",
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Untouched-period daily positive-premium one-shot LONG study")
    subs = parser.add_subparsers(dest="command", required=True)
    item = subs.add_parser("checkpoint")
    item.set_defaults(func=command_checkpoint)
    for name, function in [
        ("fetch", command_fetch),
        ("inspect", daily_parent.command_inspect),

...
每个新问题在查看结果前，按最强预期主张记录一个主要类型；类型不是阶段或质量等级：

| 类型 | 采用的最小流程 |
|---|---|
| `DESCRIPTIVE` | 固定样本、定义和估计方法，保存样本量、缺失、异常值、替代口径与时间依赖下的不确定性；可以并列多个对象的样本统计，但不推出差异可推广、因果、预测或可交易性 |
| `COMPARATIVE_OR_MECHANISM` | 当差异或排序准备推广到观察样本之外，或研究可能机制时，固定对象、指标和分组，检查分期、市场状态、替代定义和最强反解释；没有识别设计时不报告因果发现 |
| `PREDICTIVE` | 固定可用输入、预测时点、目标、简单基准和指标，使用未暴露时间区间或前进式评价，并披露完整特征、模型、窗口与指标搜索 |
| `STRATEGY_CANDIDATE` | 固定信号和下一可行动时间，进一步加入适用成本、流动性、容量、仓位和执行限制；只有获选候选才准备产品交接 |

问题混合多类主张时执行最高门槛。已经查看结果后扩大主张，原数据保持已暴露；先更新问题与 checkpoint，再取得新评价证据。所有类型继续使用同一四种结论，不增加研究状态机或登记服务。

## 共享研究环境与框架分工

本次框架选择采用的产品基准提交为 `d6cd7faa13666bcd12c2b995dcf75459f178b2ca`（`origin/main`，核对于 2026-07-21）；它不改写上方 32 个已完成问题各自记录的历史基准。

新研究默认先使用成熟第三方框架和库，Halpha 只保留问题特有的数据转换、时间对齐、必要统计或成本补充、证据门和结果导出，不自建指标库、统计平台、回测撮合核心、优化器、研究数据库或任务平台。

- VectorBT `1.1.0` 是独立研究环境的首选加速框架，用于适合数组或 bar 表达的问题的统计与滚动分析、批量候选筛选、参数与样本敏感性比较和初步组合模拟；相关、回归或专门统计方法使用问题所需的成熟统计库，没有策略动作时不创建 Portfolio。VectorBT 不进入产品构建或运行时，也不是成交和执行语义权威。
- NautilusTrader 是唯一产品量化交易核心，并用于获选策略候选的事件顺序、订单、成交、资金费、保证金和在线离线一致性验证。只有可交易主张受路径或执行依赖影响且 VectorBT 无法可靠表达时，研究才改用 NautilusTrader 或另一成熟专用组件。
...
1. 先固定主要研究类型、最强主张、问题、对象、数据边界、指标、否定条件和预计搜索范围；
2. 使用与问题匹配的成熟统计库；适合 bar/数组批量的问题用 VectorBT 一次计算完整候选矩阵，保存全部配置结果而不是只保存最佳统计或列；
3. 描述性研究报告样本内不确定性和替代口径；比较或机制研究增加分期、状态与反解释；预测和策略研究使用未暴露时间评价或 rolling/expanding walk-forward，广泛搜索时采用与指标相称的多重比较或选择偏差处理；
4. 只有策略候选才要求收盘信号移动到下一可行动时间并建模成本；单 bar 顺序、订单簿、资金费、保证金或执行反馈会改变可交易结论时转入 NautilusTrader 或明确保守代理；
5. 只有准备供所有者选择的 `STRATEGY_CANDIDATE` 才补固定策略说明和小型输入到决策轨迹；其他类型的结果只作为以后问题的证据输入，不直接交付产品策略。

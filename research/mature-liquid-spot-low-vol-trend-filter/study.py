...
PARENT_PATH = Path(__file__).resolve().parent.parent / "mature-liquid-spot-low-vol" / "study.py"
SPEC = importlib.util.spec_from_file_location("low_vol_parent", PARENT_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("cannot load retained low-volatility study")
...
def low_vol_positive_trend_target(frames, decision_ms: int, lookback: int):
    ...
    for symbol in parent.SYMBOLS:
        ...
        if len(closes) != lookback + 1:
            return {item: 0.0 for item in parent.SYMBOLS}
    ...
    low_three = sorted(volatility, key=lambda item: (volatility[item], item))[:parent.HOLDINGS]
    return {
        symbol: parent.WEIGHT if symbol in low_three and momentum[symbol] > 0 else 0.0
        for symbol in parent.SYMBOLS
    }
...
def phase_result(frames, phase: str):
    ...
    strategy = {
        str(lookback): {
            name: parent.run_with_target(frames, start_ms, end_ms, lookback, cost, 20260720 + li * 10 + ci, low_vol_positive_trend_target)
            for ci, (name, cost) in enumerate(base.COSTS.items())
        }
        for li, lookback in enumerate(parent.LOOKBACKS)
    }
    unfiltered = {
        name: parent.run_with_target(frames, start_ms, end_ms, parent.PRIMARY, cost, 20261720 + ci, parent.low_vol_target)
        for ci, (name, cost) in enumerate(base.COSTS.items())
    }
    benchmark = {
        name: parent.run_with_target(frames, start_ms, end_ms, parent.PRIMARY, cost, 20262720 + ci, parent.constant_half_long)
        for ci, (name, cost) in enumerate(base.COSTS.items())
    }
    return {"phase": phase, "period": {"start": base.PHASES[phase][0], "end_exclusive": base.PHASES[phase][1]}, "data_quality": base.data_quality(frames, start_ms, end_ms), "strategy": strategy, "unfiltered_low_vol": unfiltered, "monthly_rebalanced_half_long": benchmark}
...
def command_analyze(args):
    ...
    output.update({"schema_version": 1, "generated_at": base.utc_now(), "manifest_content_identity": manifest["content_identity"], "study_code_sha256": sha256_path(Path(__file__)), "parent_study_code_sha256": sha256_path(PARENT_PATH), "rules": {"symbols": list(parent.SYMBOLS), "lookbacks": list(parent.LOOKBACKS), "selection": "bottom 3 realized volatility then retain only own positive trend", "weight_per_eligible_asset": parent.WEIGHT, "maximum_gross": parent.GROSS, "costs": base.COSTS}})
    ...
    main = output["strategy"][str(parent.PRIMARY)]["base"]; print(json.dumps({"phase": args.phase, "base_total": main["total_return"], "max_drawdown": main["max_drawdown"]}))


def components(result):
    fixed = result["strategy"][str(parent.PRIMARY)]; return fixed, fixed["base"], result["unfiltered_low_vol"]["base"]


def command_qualify_development(args):
    result = base.read_json(Path(args.development)); fixed, main, unfiltered = components(result)
    passed = result["data_quality"]["status"] == "PASS" and main["total_return"] > 0 and fixed["stress"]["total_return"] > 0 and main["max_drawdown"] > -0.30 and main["max_drawdown"] >= unfiltered["max_drawdown"] + 0.05 and result["strategy"]["60"]["base"]["total_return"] > 0 and result["strategy"]["180"]["base"]["total_return"] > 0 and main["turnover"] <= 30
    output = {"generated_at": base.utc_now(), "development_content_digest": result["content_digest"], "qualification_status": "PASSED_DEVELOPMENT_GATE" if passed else "FAILED_DEVELOPMENT_GATE_STOP", "holdout_authorized": passed, "fixed_rule": "BOTTOM3_REALIZED_VOL_POSITIVE_TREND_90D_0P5X"}; output["content_digest"] = base.canonical_digest({key: value for key, value in output.items() if key != "generated_at"}); base.write_json(Path(args.output), output); print(json.dumps({"status": output["qualification_status"]}))


def command_qualify_evaluation(args):
    result = base.read_json(Path(args.evaluation)); fixed, main, unfiltered = components(result)
    passed = result["data_quality"]["status"] == "PASS" and main["total_return"] > 0 and fixed["stress"]["total_return"] > 0 and sum(item["return"] > 0 for item in main["by_year"].values()) >= 1 and main["max_drawdown"] > -0.30 and main["max_drawdown"] > unfiltered["max_drawdown"] and result["strategy"]["60"]["base"]["total_return"] > 0 and result["strategy"]["180"]["base"]["total_return"] > 0
...
def command_combine(args):
    development = base.read_json(Path(args.development)); evaluation = base.read_json(Path(args.evaluation)); gate = base.read_json(Path(args.evaluation_gate)); confirmation = base.read_json(Path(args.confirmation)); fixed, main, unfiltered = components(confirmation)
    ...
    support = confirmation["data_quality"]["status"] == "PASS" and main["total_return"] > 0 and fixed["stress"]["total_return"] > 0 and main["max_drawdown"] > -0.25 and main["max_drawdown"] > unfiltered["max_drawdown"] and confirmation["strategy"]["60"]["base"]["total_return"] >= 0 and confirmation["strategy"]["180"]["base"]["total_return"] >= 0 and combined > 0
    ...
    output = {"generated_at": base.utc_now(), "conclusion": conclusion, "scope": "fixed 13-symbol Binance spot monthly bottom-3 low-volatility selection with own positive-trend filter, maximum 0.5x", "development": development["strategy"][str(parent.PRIMARY)]["base"], "evaluation": evaluation["strategy"][str(parent.PRIMARY)]["base"], "confirmation": main, "confirmation_unfiltered_low_vol": unfiltered, "evaluation_confirmation_compounded_return": combined, "confirmation_support_gate": support, "formal_product_strategy_comparison": "NOT_RUN_NO_COMPARABLE_INSTRUMENT_OR_ACTIVATION_REPLAY", "product_effects": "NONE"}; output["content_digest"] = base.canonical_digest({key: value for key, value in output.items() if key != "generated_at"}); base.write_json(Path(args.output), output); print(json.dumps({"conclusion": conclusion, "combined": combined}))


def build_parser():
    parser = argparse.ArgumentParser(description="Low-volatility spot selection with positive-trend filter"); sub = parser.add_subparsers(dest="command", required=True)

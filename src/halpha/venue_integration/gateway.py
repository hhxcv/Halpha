...
from halpha.venue_integration.transitions import validate_execution_action_shape
...
class PersistedActionGate:
    ...
    def _require_identity(self, action: ExecutionAction) -> None:
        ...
        if action.action_kind is ExecutionActionKind.CANCEL:
            ...
            target = (
                self._repository.find_order_action_by_client_id(
            ...
            if (
                target is None
                or target.action_kind is ExecutionActionKind.CANCEL
                or target.client_order_id != target_client_order_id
                or target.environment_id != action.environment_id
                or target.activation_id != action.activation_id
                or target.account_ref != action.account_ref
                or target.action_terms.get("instrument_ref")
                != action.action_terms.get("instrument_ref")
            ):
                raise RuntimeError("AUTHORIZATION_MISMATCH")

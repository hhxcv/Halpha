...
BINANCE_USDM_VENUE_REF = "BINANCE_USDM"
...
class ExitResponsibilityRole(StrEnum):
    PRIMARY_EXIT = "PRIMARY_EXIT"
    EXIT_SUCCESSOR = "EXIT_SUCCESSOR"
    POST_SUCCESSOR_LATE_ENTRY_CLEANUP = "POST_SUCCESSOR_LATE_ENTRY_CLEANUP"
...
class VenueFactAttributionClass(StrEnum):
    ...
    HALPHA_ACTIVATION_ALLOCATION = "HALPHA_ACTIVATION_ALLOCATION"
...
class ExecutionAction(VenueModel):
    ...
    def invariants_hold(self) -> ExecutionAction:
        ...
        if self.action_kind is not ExecutionActionKind.CANCEL and self.cancel_target is not None:
            raise ValueError("CANCEL_TARGET_INVALID")
        exit_role = self.action_terms.get("exit_responsibility_role")
        if self.action_kind is ExecutionActionKind.EXIT:
            try:
        ...
        elif self.state is ExecutionActionState.HANDED_OVER:
...
class VenueFact(VenueModel):
    ...
    def fact_invariants_hold(self) -> VenueFact:
        ...
        if self.attribution_class is None:
            ...
            is VenueFactAttributionClass.HALPHA_ACTIVATION_ALLOCATION
        ):
            if (
                self.activation_ref is None

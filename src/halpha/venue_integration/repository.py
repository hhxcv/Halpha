...
from halpha.venue_integration.facts import venue_trade_fact_is_canonicalizable
from halpha.venue_integration.models import ExecutionAction, VenueFact, VenueFactKind
...
class PostgreSQLExecutionActionRepository:
        ...
        row = self._connection.execute(
            ...
            + """
              WHERE environment_id = %s AND client_order_id = %s
                AND (
                    state IN (
                        'SUBMITTING', 'UNKNOWN', 'OPEN', 'NOT_SUBMITTED', 'CLOSED'
                    )
                    OR (state = 'HANDED_OVER' AND request_digest IS NOT NULL)
                )
            """,
    ...
    def find_order_action_by_venue_order_ref(
        self,
        venue_order_ref: str,
    ) -> ExecutionAction | None:
        """Recover a called Halpha order when a framework invents a new client ID."""
        ...
        row = self._connection.execute(
            ...
            + """
              ...
              LIMIT 1
            """,
            (self._environment_id, venue_order_ref),
        ).fetchone()
        return None if row is None else _action_from_row(row)
    ...
    def list_open_cancels_for_target(
        self,
        client_order_id: str,
...
class PostgreSQLVenueFactRepository:
    ...
    def list_trade_versions(self, fact: VenueFact) -> tuple[VenueFact, ...]:
        if fact.kind not in {VenueFactKind.FILL, VenueFactKind.COMMISSION}:
            raise VenueIntegrationConflict("VENUE_TRADE_FACT_REQUIRED")
        rows = self._connection.execute(
            _FACT_SELECT
            + """
              WHERE environment_id = %s
                ...
                AND instrument_ref IS NOT DISTINCT FROM %s
    ...
    def find_by_source(self, fact: VenueFact) -> VenueFact | None:
        if venue_trade_fact_is_canonicalizable(fact):
            candidates = self.list_trade_versions(fact)

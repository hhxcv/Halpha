...
def build_decision_intelligence_delta(
    ...
    *,
    ...
    return [DECISION_INTELLIGENCE_DELTA_ARTIFACT]
...
def build_decision_intelligence_material(config: dict[str, Any], run: RunContext) -> list[str]:
    ...
    decision_records = _records_from_artifact(
        inputs["decision_recommendations"],
        DECISION_RECOMMENDATIONS_ARTIFACT,
    ...
    run.manifest["counts"]["decision_intelligence_material_records"] = len(decision_records)
...
def _watch_trigger_conditions_by_tuple(records: list[dict[str, Any]]) -> dict[tuple[str, str, str], list[str]]:
    ...
    return {
        key: sorted(_unique_ordered(values))

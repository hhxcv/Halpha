...
def macro_calendar_history_storage_path(config_path: Path) -> Path:
    return config_path.parent / MACRO_CALENDAR_HISTORY_STORAGE_ARTIFACT


def macro_calendar_history_schema_path(config_path: Path) -> Path:
    return config_path.parent / MACRO_CALENDAR_HISTORY_SCHEMA_ARTIFACT


def macro_calendar_history_state_path(config_path: Path) -> Path:
    return config_path.parent / MACRO_CALENDAR_HISTORY_STATE_ARTIFACT
...
def _history_record(item: dict[str, Any], run: RunContext, *, observed_at: str) -> dict[str, Any]:
    ...
    return {
        ...
        "warnings": warnings,
        "errors": errors,
        "source_artifacts": [display_path(run.raw_dir / "macro_calendar.json", base=run.config_path.parent)],
    }
...
def _write_schema(schema_path: Path, run: RunContext) -> None:
    schema = {
        ...
        "storage_path": display_path(macro_calendar_history_storage_path(run.config_path), base=run.config_path.parent),
...
def _skipped_state(run: RunContext, *, reason: str, now: datetime | str | None) -> dict[str, Any]:
    ...
    return {
        ...
        "schema_path": display_path(macro_calendar_history_schema_path(run.config_path), base=run.config_path.parent),
        "state_path": display_path(state_path, base=run.config_path.parent),
        "totals": {
            "records": 0,
            "incoming_records": 0,

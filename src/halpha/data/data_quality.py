...
def _run_index_check(run: RunContext) -> dict[str, Any]:
    ...
    if not isinstance(summary, dict):
        return _check(
            ...
            "shared_data",
            "skipped",
            "run index is a terminal state projection committed after the final manifest; this stage-time skip is expected and must not be reported as a final missing run index.",
            [artifact],
            details={
                "terminal_state_projection": True,
                "committed_after_final_manifest": True,

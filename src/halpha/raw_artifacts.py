...
def validate_macro_calendar_raw_artifact(raw: Any, artifact: str) -> None:
    items = _items(raw, artifact)
    for index, item in enumerate(items):
        path = f"items[{index}]"
        _required_string(item, "item_id", artifact, path)
        _required_string(item, "data_class", artifact, path)
        _required_string(item, "source", artifact, path)
        _required_string(item, "event_name", artifact, path)
        _required_string(item, "event_type", artifact, path)
        _required_string(item, "region", artifact, path)
        _required_string(item, "scheduled_at", artifact, path)
        _required_string(item, "source_timezone", artifact, path)
        _required_string(item, "importance", artifact, path)
        _required_string(item, "endpoint", artifact, path)
        _required_mapping(item, "metrics", artifact, path)
        _required_mapping(item, "units", artifact, path)
        _required_mapping(item, "raw_fields", artifact, path)

...
def effective_leverage(actual_margin_mode: str, actual_leverage: str) -> Decimal:
    """Return conservative leverage arithmetic without mutating the account."""
...
def check_action(
    ...
    *,
    ...
    # Cancelling one exact Halpha-owned order does not need a fresh valuation or
    # a position-wide attribution proof. Keeping that order live while a market
    # or account read is stale can increase risk; the persisted cancel target is
    # the relevant ownership proof at the EXE boundary.

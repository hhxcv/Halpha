...
def mutex_grants(
    ...
    identities = (
        ...
        task_sid,
        *((maintenance_sid,) if maintenance_sid is not None else ()),
    )
    if len(set(identities)) != len(identities):
    ...
    if maintenance_sid is not None:
        grants[maintenance_sid] = MUTEX_ALL_ACCESS
    return grants
...
def _acquire_exclusive_mutex(
    *,
    ...
    attributes = _security_attributes(owner_sid=owner_sid, grants=grants)
    try:
    ...
    if already_exists:
        handle.Close()
        raise WindowsRuntimeError(conflict_code)
    try:
        assert_kernel_object_security(handle, owner_sid=owner_sid, grants=grants)
    except Exception:
...
def acquire_executor_mutex(
    *,
    ...
    require_process_identity(task_sid)
    return _acquire_exclusive_mutex(
        name=name,
        owner_sid=task_sid,
        grants=mutex_grants(task_sid, maintenance_sid),
        conflict_code="WINDOWS_EXECUTOR_MUTEX_ALREADY_HELD",
    )


def acquire_executor_maintenance_mutex(
    *,
    name: str,
    executor_task_sid: str,
    maintenance_sid: str,
    conflict_code: str = "EXECUTOR_MUST_BE_STOPPED_FOR_MAINTENANCE",
) -> ExecutorWriteMutex:
    """Exclude the environment's Executor while maintenance mutates its state."""

    require_process_identity(maintenance_sid)
    return _acquire_exclusive_mutex(
        name=name,
        owner_sid=maintenance_sid,
        grants=mutex_grants(executor_task_sid, maintenance_sid),
        conflict_code=conflict_code,
    )


def acquire_live_gate_maintenance_mutex(
    *,
    name: str,
    executor_task_sid: str,
    maintenance_sid: str,
) -> ExecutorWriteMutex:
    """Exclude a running Executor while maintenance replaces the Live gate."""

    return acquire_executor_maintenance_mutex(
        name=name,
        executor_task_sid=executor_task_sid,
        maintenance_sid=maintenance_sid,
        conflict_code="LIVE_WRITE_EXECUTOR_MUST_BE_STOPPED",
    )

...
_REDUCING_ACTIONS = frozenset(
    {"PROTECTION", "TAKE_PROFIT", "RISK_REDUCTION", "EXIT", _EXTERNAL_CLOSURE}
)
...
def summarize_trade_result(
    *,
    ...
    opening_position_quantity: str | None = None,
) -> dict[str, Any]:
    """Return P&L only for Halpha-owned entries with complete fee evidence.

    ``opening_position_quantity`` marks an external-position disposition.  It
    makes remaining quantity and operational closure observable without ever
    manufacturing an ENTRY or strategy P&L for the pre-existing position.
    """
    ...
    funding_conflicts: set[str] = set()
    valid = direction in {"LONG", "SHORT"}
    external_position_disposition = opening_position_quantity is not None
    opening_quantity = Decimal("0")
    if external_position_disposition:
        try:
            opening_quantity = _positive_decimal(opening_position_quantity)
        except ValueError:
            valid = False
    materialized_facts = tuple(facts)
    synthetic_trade_aliases = _synthetic_trade_id_aliases(materialized_facts)
    for fact in materialized_facts:
        ...
        if not isinstance(payload, Mapping):
            if kind in {"FILL", "COMMISSION"}:
                valid = False
            continue
        raw_trade_id = str(payload.get("trade_id", ""))
        if kind == "COMMISSION" and raw_trade_id in synthetic_trade_aliases:
            continue
        trade_id = synthetic_trade_aliases.get(raw_trade_id, raw_trade_id)
        if kind == "FILL":
            ...
            try:
                income = _decimal(payload.get("income"))
            except ValueError:
                valid = False
                continue
            if (
                payload.get("record_type")
                != "ACTIVATION_FUNDING_ALLOCATION"
                or payload.get("currency") != "USDT"
                or not transaction_id
            ):
                valid = False
            elif (
                transaction_id in funding
                and funding[transaction_id] != income
            ):
                valid = False
                funding_conflicts.add(transaction_id)
            else:
                funding[transaction_id] = income

    position_quantity = (
        opening_quantity if direction == "LONG" else -opening_quantity
    )
    ...
    funding_total = sum(funding.values(), Decimal(0))
    commission_complete = (
        bool(fills)
        and set(fills) == set(commissions)
        and not set(fills).intersection(commission_conflicts)
    )
    execution_cost_complete = valid and commission_complete
    calculation_complete = execution_cost_complete and not external_position_disposition
    closed = bool(fills) and position_quantity == 0
    gross_pnl = fill_cash_flow if closed and calculation_complete else None
    net_pnl = (
        gross_pnl - commission_total + funding_total
    ...
    return {
        ...
        "funding": canonical_decimal(funding_total),
        "funding_record_count": len(funding),
        "funding_complete": bool(funding) and not funding_conflicts,
        ...
        "strategy_attribution_complete": (
            not external_position_disposition and external_closure_count == 0
        ),
    }


def _synthetic_trade_id_aliases(
    facts: tuple[Mapping[str, Any], ...],
) -> dict[str, str]:
    grouped: dict[tuple[object, ...], list[tuple[str, bool]]] = {}
    for fact in facts:
        if str(fact.get("kind", "")) != "FILL":
            continue
        payload = fact.get("payload")
        if not isinstance(payload, Mapping):
            continue
        ...
        try:
            key = (
                str(fact.get("action_ref", "")),
                str(payload.get("client_order_id", "")),
                str(payload.get("venue_order_ref", "")),
                _source_datetime(fact.get("source_time")),
                _positive_decimal(payload.get("last_price")),
                _positive_decimal(payload.get("last_quantity")),
                str(payload.get("order_side", "")).upper(),
            )
        except ValueError:
            continue
        grouped.setdefault(key, []).append(
            (trade_id, _synthetic_query_trade_id(trade_id, payload))
        )
    aliases: dict[str, str] = {}
    for candidates in grouped.values():
        authoritative = sorted(
            trade_id
            for trade_id, synthetic in candidates
            if trade_id and not synthetic
        )
        ...
        for trade_id, synthetic in candidates:
            if synthetic:
                aliases[trade_id] = target
    return aliases


def _synthetic_query_trade_id(
    trade_id: str,

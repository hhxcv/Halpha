...
MAX_DECIMAL_ABS_ADJUSTED_EXPONENT = 128
...
def _decimal_within_resource_bounds(value: Decimal) -> bool:
    """Bound rendering cost without imposing a product-specific money scale."""

    return (
        len(value.as_tuple().digits) <= MAX_DECIMAL_SIGNIFICANT_DIGITS
        and abs(value.adjusted()) <= MAX_DECIMAL_ABS_ADJUSTED_EXPONENT
    )
...
def decimal_from_string(
    ...
    *,
    ...
    if not parsed.is_finite() or not _decimal_within_resource_bounds(parsed):
        raise DomainValidationError(code)
...
def canonical_decimal(value: Decimal) -> str:
    ...
    if not _decimal_within_resource_bounds(value):
        raise DomainValidationError("DECIMAL_OUT_OF_RANGE")

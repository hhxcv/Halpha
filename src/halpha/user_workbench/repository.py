...
class PostgreSQLCommandRepository:
    ...
    def list_processing_target_refs(self) -> tuple[str, ...]:
        rows = self._connection.execute(
            """
            SELECT DISTINCT c.target_ref
            FROM halpha.command c
            JOIN halpha.receipt r
    ...
    def insert(self, command: Command, receipt: Receipt) -> None:
        if (
            command.environment_id != self._environment_id
            or receipt.environment_id != self._environment_id
        ):
            raise CommandConflict("COMMAND_ENVIRONMENT_MISMATCH")
    ...
    def update_receipt(self, receipt: Receipt, *, expected_version: int) -> None:
        if receipt.environment_id != self._environment_id:
            raise CommandConflict("COMMAND_ENVIRONMENT_MISMATCH")

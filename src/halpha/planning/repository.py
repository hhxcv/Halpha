    ...
    def insert_activation(self, activation: PlanActivation) -> None:
        self._connection.execute(
            ...
            INSERT INTO halpha.plan_activation (
                ...
                plan_version_ref, authorization_version_ref, allocation_ref,
                account_ref, instrument_ref, direction, strategy_id, framework_strategy_id,
                target_exposure,
    ...
    def get_activation(
        ...
        row = self._connection.execute(
            ...
            SELECT activation_id, environment_id, environment_kind, authority_class,
                   plan_version_ref, authorization_version_ref, allocation_ref,
                   account_ref, instrument_ref, direction, strategy_id,
        ...
        return PlanActivation(
            ...
            authorization_version_ref=str(row[5]),
            allocation_ref=str(row[6]),
            account_ref=str(row[7]),
            instrument_ref=str(row[8]),
            direction=str(row[9]),
            strategy_id=str(row[10]),
            framework_strategy_id=str(row[11]),
            target_exposure=str(row[12]),
